...
import { BarChart3, BookOpen, Boxes, Flame, Headphones, Inbox, LayoutDashboard, LifeBuoy, LogOut, Settings, Ticket, X, } from 'lucide-react';
 ...
 */
function entryActive(id, pathname, search) {
...
function Brand({ compact }) {
    return (<div className="flex h-16 items-center gap-2.5 px-5">
...
function Item({ entry, active, compact, onNavigate }) {
    const Icon = entry.icon;
    const classes = cn('group relative flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium transition-colors', active ? 'bg-brand-50 font-semibold text-brand-700' : 'text-ink-muted hover:bg-surface-sunken hover:text-ink', compact && 'justify-center px-0');
    const body = (<>
      {active && !compact && (<span className="absolute -left-3 top-1/2 h-6 w-1 -translate-y-1/2 rounded-r-full bg-brand-600" aria-hidden="true"/>)}
      ...
      {!compact && entry.badge && (<span className="ml-auto inline-flex min-w-6 items-center justify-center rounded-full bg-brand-900 px-1.5 py-0.5 text-2xs font-bold tabular text-white">
          {entry.badge}
        </span>)}
    </>);
    /* Hash anchors are plain jumps — they never take the active pill. */
    if (entry.hash) {
        return (<a href={entry.to} onClick={onNavigate} title={compact ? entry.label : undefined} className={classes}>
        {body}
      </a>);
    }
    return (<Link to={entry.to} onClick={onNavigate} title={compact ? entry.label : undefined} aria-current={active ? 'page' : undefined} className={classes}>
      {body}
    </Link>);
}
function SectionLabel({ compact, children }) {
    if (compact)
        return <div className="mx-3 my-2 h-px w-auto bg-line" aria-hidden="true"/>;
    return (<p className="mb-1.5 mt-5 px-3 text-[0.6875rem] font-semibold uppercase tracking-[0.08em] text-ink-subtle first:mt-1">
      {children}
    </p>);
}
function LogoutItem({ compact, onNavigate }) {
    const logout = useAuthStore((state) => state.logout);
    const navigate = useNavigate();
    return (<button type="button" title={compact ? 'Logout' : undefined} onClick={() => {
            onNavigate?.();
            void logout()
                .catch(() => undefined)
                .finally(() => navigate('/login'));
        }} className={cn('flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium text-ink-muted transition-colors hover:bg-surface-sunken hover:text-ink', compact && 'justify-center px-0')}>
 ...
 */
function HelpButton({ compact, onNavigate }) {
    const setShortcutHelpOpen = useUiStore((state) => state.setShortcutHelpOpen);
    return (<button type="button" title={compact ? 'Help' : undefined} onClick={() => {
            onNavigate?.();
...
function Nav({ compact, onNavigate }) {
    ...
    const withLiveBadge = (entries) => entries.map((entry) => entry.id === 'tickets' && typeof openCount === 'number'
        ? { ...entry, badge: String(openCount) }
        : entry);
    const visible = (entries) => entries.filter((entry) => !entry.permission || can(entry.permission));
    const menu = withLiveBadge(visible(MENU));
    const general = visible(GENERAL);
    return (<nav className="flex flex-1 flex-col overflow-y-auto px-3 pb-4" aria-label="Primary">
      <SectionLabel compact={compact}>Menu</SectionLabel>
      {menu.map((entry) => (<Item key={entry.id} entry={entry} active={entryActive(entry.id, pathname, search)} compact={compact} onNavigate={onNavigate}/>))}

      <SectionLabel compact={compact}>General</SectionLabel>
      {general.map((entry) => (<Item key={entry.id} entry={entry} active={entryActive(entry.id, pathname, search)} compact={compact} onNavigate={onNavigate}/>))}
      <HelpButton compact={compact} onNavigate={onNavigate}/>
      <LogoutItem compact={compact} onNavigate={onNavigate}/>
    </nav>);
}
function PromoCard({ compact }) {
    if (compact)
        return null;
    return (<div className="px-3 pb-4">
      <div className="relative overflow-hidden rounded-2xl bg-brand-800 p-4 text-white shadow-card">
        <div className="pointer-events-none absolute inset-0" aria-hidden="true" style={{
            background: 'radial-gradient(120% 90% at 85% 110%, rgb(255 255 255 / 0.14) 0%, transparent 55%), radial-gradient(100% 80% at 10% -10%, rgb(255 255 255 / 0.10) 0%, transparent 50%)',
        }}/>
        ...
        <Link to="/" className="relative mt-3 block rounded-full bg-white/95 px-3 py-1.5 text-center text-xs font-semibold text-brand-800 transition-colors hover:bg-white">
          Open overview
        </Link>
      </div>
    </div>);
...
export function Sidebar() {
    ...
    return (<>
      ...
      <aside className={cn('fixed inset-y-0 left-0 z-30 hidden flex-col border-r border-line bg-surface lg:flex', 'transition-[width] duration-200', collapsed ? 'w-sidebar-collapsed' : 'w-sidebar')}>
      ...
      {mobileOpen && (<div className="lg:hidden">
          <div className="overlay-backdrop" onClick={close} role="presentation" aria-hidden="true"/>
          <aside className="fixed inset-y-0 left-0 z-50 flex w-sidebar flex-col border-r border-line bg-surface" role="dialog" aria-modal="true" aria-label="Navigation">
