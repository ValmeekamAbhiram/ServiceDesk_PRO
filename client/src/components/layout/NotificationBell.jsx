...
const QUERY = { page: 1, limit: 8, unreadOnly: false };
export function NotificationBell({ circle = false }) {
    ...
    return (<div className="relative" ref={wrapper}>
      <button type="button" className={circle
            ...
            : 'btn btn-ghost btn-icon relative'} aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
        <Bell className="h-4 w-4" strokeWidth={1.75} aria-hidden="true"/>
        {unread > 0 && (<span className="absolute right-1 top-1 grid min-w-[1.05rem] place-items-center rounded-full bg-danger-solid px-1 text-[0.625rem] font-bold leading-4 text-white">
            {unread > 99 ? '99+' : unread}
          </span>)}
      ...
      {open && (<div className="popover absolute right-0 mt-2 w-80 max-w-[calc(100vw-1.5rem)]" role="menu" aria-label="Notifications">
          <div className="flex items-center justify-between px-2.5 py-1.5">
            <span className="text-xs font-semibold text-ink">Notifications</span>
            {unread > 0 && (<button type="button" className="btn btn-ghost btn-sm" disabled={markAll.isPending} onClick={() => markAll.mutate()}>
          ...
          {list.isLoading ? (<div className="flex justify-center py-6">
              <Spinner className="h-4 w-4 text-brand-600"/>
            </div>) : items.length === 0 ? (<p className="px-3 py-6 text-center text-xs text-ink-subtle">Nothing yet.</p>) : (<ul className="max-h-80 overflow-y-auto py-1">
              {items.map((item) => (<li key={item.id}>
                  <button type="button" role="menuitem" className={cn('menu-item items-start', !item.read && 'bg-brand-50/60')} onClick={() => {
                        if (!item.read)
                    ...
                    <span className={cn('dot mt-1.5', item.read ? 'bg-line-strong' : 'bg-brand-500')} aria-hidden="true"/>
          ...
          <div className="divider"/>
          <button type="button" role="menuitem" className="menu-item justify-center text-xs font-medium text-brand-600" onClick={() => {
                setOpen(false);
