...
export function TechnicianLoadTable({ rows }) {
      ...
      <table className="table table-compact">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (<tr key={row.technician.id}>
                <td>
                ...
                <td>
                  <div className="h-1.5 min-w-28 overflow-hidden rounded-full bg-surface-sunken" role="img" aria-label={`${row.technician.name}: ${workload}% of the highest open workload shown`}>
                    <div className={cn('h-full min-w-px rounded-full', row.breached > 0 ? 'bg-danger-solid' : 'bg-brand-600')} style={{ width: `${workload}%` }}/>
                  </div>
                </td>
                ...
                <td className={cn('tabular text-right text-xs font-semibold', row.breached > 0 ? 'text-danger-fg' : 'text-ink-subtle')}>
                  {row.breached}
                </td>
