...
import type { SlaTargetDto } from '@shared/types';
 ...
 */
function useSlaTick(target: SlaTargetDto) {
  const now = useNow();
  ...
  const remainingMs =
    settled || target.remainingMs === null
      ? target.remainingMs
  ...
  return (
    <div className={cn('space-y-1.5', className)}>
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-surface-sunken" role="presentation">
        <div
          className={cn('h-full rounded-full transition-[width] duration-500', TONE_SOLID[meta.tone])}
          style={{ width: `${percent}%` }}
        />
      </div>
 ...
 */
export function SlaChip({ target, className }: { target: SlaTargetDto; className?: string }) {
  const { remainingMs, settled, overdue, meta } = useSlaTick(target);
