...
export function TicketTimeline({ history }) {
    ...
    return (<ol className="timeline-rail space-y-4">
      {entries.map((entry, index) => (<li key={`${entry.at}-${index}`} className="flex gap-3">
          ...
          <span className={cn('mt-1 ml-1.5 h-2.5 w-2.5 shrink-0 rounded-full ring-4 ring-surface', TONE_SOLID[ticketStatusMeta(entry.to).tone])} aria-hidden="true"/>
          <div className="min-w-0 flex-1 space-y-1">
            <p className="flex flex-wrap items-center gap-1.5 text-xs text-ink-muted">
              {entry.from === null ? (<>Raised as <StatusBadge status={entry.to}/></>) : (<>
