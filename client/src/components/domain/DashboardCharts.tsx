...
import { priorityMeta, ticketStatusMeta, type Tone } from '@shared/labels';
import type { CountByKeyDto, TimeSeriesPointDto } from '@shared/types';
...
function tooltipDate(label: unknown, payload: ReadonlyArray<{ payload?: { date?: string } }>): string {
  const iso = payload[0]?.payload?.date;
  if (!iso) return String(label);
  const date = new Date(`${iso}T00:00:00`);
  return Number.isNaN(date.getTime())
    ? String(label)
...
export function VolumeChart({ points }: { points: TimeSeriesPointDto[] }) {
  if (points.length === 0) {
      ...
      <div className="h-56 w-full sm:h-64" aria-hidden="true">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: -14 }}>
            ...
            />
            <Tooltip
              cursor={{ stroke: AXIS, strokeDasharray: '3 3', strokeWidth: 1 }}
            ...
            <Area
              ...
              strokeWidth={2}
              fill="url(#sdp-created)"
              activeDot={{ r: 4, strokeWidth: 2, stroke: 'rgb(var(--c-surface-raised))' }}
            />
            <Area
              type="monotone"
              dataKey="resolved"
              name="Resolved"
              stroke={TONE_VAR.success}
              strokeWidth={2}
              fill="url(#sdp-resolved)"
              activeDot={{ r: 4, strokeWidth: 2, stroke: 'rgb(var(--c-surface-raised))' }}
            />
          </AreaChart>
        </ResponsiveContainer>
...
function toneFor(row: CountByKeyDto, kind: 'status' | 'priority' | 'category'): Tone {
  if (kind === 'status') return ticketStatusMeta(row.key as TicketStatus).tone;
  if (kind === 'priority') return priorityMeta(row.key as Priority).tone;
  return 'primary';
}
        ...
        return (
          <li key={row.key}>
            ...
            <div
              ...
              role="img"
              aria-label={`${row.label}: ${row.count}, ${share}% of shown total`}
            >
              <div
                className={cn('h-full min-w-px rounded-full', TONE_SOLID[toneFor(row, kind)])}
                style={{
                  width: `${width}%`,
