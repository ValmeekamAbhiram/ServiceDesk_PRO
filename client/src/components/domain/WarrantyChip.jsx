...
export function WarrantyChip({ asset }) {
    const days = asset.warrantyDaysRemaining;
    if (days === null)
        return <span className="text-xs text-ink-subtle">Not recorded</span>;
    if (days < 0) {
        const ago = Math.abs(days);
        return <Badge tone="danger">Expired {ago === 1 ? 'yesterday' : `${ago} days ago`}</Badge>;
    }
    if (days === 0)
        return <Badge tone="danger">Expires today</Badge>;
    if (days <= 30)
        return <Badge tone="warning">{days} days left</Badge>;
    if (days <= 90)
