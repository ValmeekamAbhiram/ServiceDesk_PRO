...
import type { TicketCommentDto } from '@shared/types';
...
function CommentCard({ comment, now }: { comment: TicketCommentDto; now: number }) {
  ...
  return (
    <li
      className={cn(
        ...
        internal ? 'border-warning-border bg-warning-bg' : 'border-line bg-surface'
      )}
    >
...
export function CommentThread({
  ...
  loading: boolean;
  /** False once a ticket is closed — the server refuses the post either way. */
  canReply: boolean;
}) {
  const now = useNow();
  ...
  const post = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof ApiClientError ? error.message : 'That reply could not be posted.'
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        ...
        <p className="text-xs text-ink-subtle">No replies yet.</p>
      ) : (
      ...
      {canReply ? (
        <div className="space-y-3 rounded-card border border-line bg-surface-sunken p-3">
          <Textarea
            ...
            placeholder={internal ? 'A note for the team only…' : 'Add a reply…'}
            aria-label="Reply"
          />
        ...
        <p className="rounded-card border border-line bg-surface-sunken px-4 py-3 text-xs text-ink-subtle">
