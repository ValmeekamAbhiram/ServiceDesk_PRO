...
export function AttachmentPicker({ id = 'files', files, onChange, label = 'Attach a file', }) {
    ...
    const add = (picked) => {
        ...
        for (const file of Array.from(picked)) {
            ...
            const already = [...files, ...accepted].some((existing) => existing.name === file.name && existing.size === file.size);
            if (!already)
                accepted.push(file);
        }
        const room = Math.max(MAX_FILES - files.length, 0);
        if (accepted.length > room)
            toast.warning(`At most ${MAX_FILES} files can go on one post.`);
    ...
    return (<div className="space-y-2">
      <input ref={inputRef} id={id} type="file" multiple accept={ACCEPT_ATTRIBUTE} className="sr-only" onChange={(event) => add(event.target.files)}/>
      <Button variant="ghost" size="sm" onClick={() => inputRef.current?.click()} disabled={files.length >= MAX_FILES}>
        <Paperclip className="h-4 w-4" aria-hidden="true"/>
        {label}
      </Button>
      <ul className="space-y-1.5">
        {files.map((file) => (<li key={`${file.name}-${file.size}`} className="flex items-center gap-2 text-xs">
            <span className="truncate text-ink">{file.name}</span>
            <span className="tabular text-ink-subtle">{formatBytes(file.size)}</span>
            <button type="button" className="ml-auto rounded p-1 text-ink-subtle hover:text-danger-fg" onClick={() => onChange(files.filter((candidate) => candidate !== file))} aria-label={`Remove ${file.name}`}>
