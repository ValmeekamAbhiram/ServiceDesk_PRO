...
export function TicketRow({ ticket, now, showRequester = true, }) {
    const navigate = useNavigate();
    return (<tr onClick={() => navigate(`/tickets/${ticket.id}`)} className="cursor-pointer transition-colors hover:bg-brand-50">
      <td className="whitespace-nowrap">
        <Link to={`/tickets/${ticket.id}`} className="font-mono text-xs font-semibold text-brand-600 hover:underline" onClick={(event) => event.stopPropagation()}>
          {ticket.number}
        </Link>
      </td>
      ...
      <td className="min-w-[16rem] max-w-[26rem]">
        ...
        <span className="mt-0.5 flex items-center gap-2.5 text-2xs text-ink-subtle">
          ...
          {ticket.commentCount > 0 && (<span className="inline-flex items-center gap-1">
              <MessageSquare className="h-3 w-3" aria-hidden="true"/>
              {ticket.commentCount}
            </span>)}
          {ticket.attachmentCount > 0 && (<span className="inline-flex items-center gap-1">
      ...
      {showRequester && (<td className="whitespace-nowrap">
          {ticket.requester ? (<span className="flex items-center gap-2">
              <Avatar name={ticket.requester.name} size="xs"/>
              <span className="truncate text-xs text-ink-muted">{ticket.requester.name}</span>
            </span>) : (<span className="text-xs text-ink-subtle">—</span>)}
        </td>)}

      <td className="whitespace-nowrap">
        {ticket.assignee ? (<span className="flex items-center gap-2">
            <Avatar name={ticket.assignee.name} size="xs"/>
            <span className="truncate text-xs text-ink-muted">{ticket.assignee.name}</span>
          </span>) : (<span className="text-xs text-ink-subtle">Unassigned</span>)}
      </td>
