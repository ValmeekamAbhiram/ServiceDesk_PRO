...
export function Avatar({ name, size = 'md', className, }) {
    ...
    return (<span title={label} style={{ backgroundColor: avatarColorFor(label) }} className={cn('inline-flex shrink-0 select-none items-center justify-center rounded-full font-semibold uppercase tracking-wide text-white', dimensions, className)}>
      {initialsOf(label)}
    </span>);
}
