...
export function Skeleton({ className }) {
    return (<div className={cn('relative overflow-hidden rounded-md bg-surface-sunken', className ?? 'h-4 w-full')} aria-hidden="true">
...
export function SkeletonRows({ rows = 5, cols = 4 }) {
    return (<>
      {Array.from({ length: rows }).map((_, rowIndex) => (<tr key={rowIndex} className="border-b border-line last:border-0">
          {Array.from({ length: cols }).map((__, colIndex) => (<td key={colIndex} className="px-4 py-3">
