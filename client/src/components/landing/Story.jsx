...
function TicketRow({ id, title, color, tag }) {
    return (<div className="ld-ticket-row">
      <span>
...
function Chapter({ n, title }) {
    return (<p className="ld-chapter" style={{ margin: '0 0 10px' }}>
      {n} — {title}
    </p>);
}
/** Marquee divider strip between chapters: reuses ticker style, aria-hidden duplicate. */
function MarqueeDivider({ text }) {
    return (<div className="ld-marquee ld-divider" aria-hidden="true">
...
function SlaCountdown() {
    ...
    if (reduce) {
        return (<div className="ld-sla" role="img" aria-label="SLA countdown from 4 hours to breached">
    ...
    return (<div ref={ref} className="ld-sla" role="img" aria-label="SLA countdown from 4 hours to breached">
      <motion.div className={breached ? 'ld-sla-time breached ld-mono' : 'ld-sla-time ld-mono'} aria-hidden="true">
        {label}
      </motion.div>
...
export function Story() {
    return (<>
        ...
        <div className="ld-wrap">
          <Reveal>
            ...
            <div className="ld-flywheel">
              {['🎫 Resolve TKT-1042', '📝 Write KB-118 v2', '✦ Deflect 32% chats', '📉 −30 tickets / week'].map((s) => (<div key={s} className="ld-fw-step">
                    {s}
                  </div>))}
            </div>
          </Reveal>
      ...
      <section id="story-9" className="ld-section block-green">
        <div className="ld-wrap">
          <Reveal>
        ...
        ].map(([v, l]) => (<div key={l} className="ld-stat">
                  <b>{v}</b>
                  <small>{l}</small>
                </div>))}
            </div>
            <div className="ld-btn-row">
              ...
              <Link to="/login" className="ld-btn" style={{ background: 'transparent', color: '#fff', borderColor: '#fff' }}>
                Sign in
              </Link>
            </div>
          </Reveal>
