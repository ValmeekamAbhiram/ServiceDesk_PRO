...
export interface PaletteResult {
  id: string;
  group: 'Tickets' | 'Assets' | 'Knowledge' | 'People';
  label: string;
  description: string;
  to: string;
}

const GROUP_ICON: Record<PaletteResult['group'], ReactNode> = {
...
export function PaletteResults({
  ...
  let previousGroup: PaletteResult['group'] | null = null;
  return (
    <div
      id="command-results"
      role="listbox"
      ...
      {results.map((result, index) => {
        ...
        return (
          <div key={`${result.group}-${result.id}`}>
            {showGroup && (
              <p className="px-2.5 pb-1 pt-2 text-2xs font-semibold uppercase tracking-wider text-ink-subtle">
                {result.group}
              </p>
            )}
            <button
              ...
              className={cn(
                ...
                index === activeIndex ? 'bg-brand-50 text-brand-700' : 'text-ink hover:bg-surface-sunken'
              )}
