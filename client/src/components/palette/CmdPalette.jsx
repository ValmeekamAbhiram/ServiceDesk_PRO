...
export function CmdPalette() {
    ...
    return createPortal(<div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-950/60 p-3 pt-[8vh] backdrop-blur-sm sm:p-6 sm:pt-[12vh]" onMouseDown={(event) => {
      ...
      <section role="dialog" aria-modal="true" aria-label="Command search" className="w-full max-w-2xl overflow-hidden rounded-panel border border-line bg-surface-raised shadow-pop">
        ...
        }} placeholder="Search tickets, assets, knowledge, or people…" aria-label="Search the service desk" aria-controls="command-results" aria-activedescendant={results[activeIndex] ? `command-result-${activeIndex}` : undefined} className="h-14 min-w-0 flex-1 bg-transparent text-base text-ink outline-none placeholder:text-ink-subtle"/>
