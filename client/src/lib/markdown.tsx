...
export const SUPPORTED =
  '# heading · **bold** · *italic* · `code` · - list · 1. list · > quote · ```fenced``` · [link](https://…)';

const SAFE_SCHEME = /^(https?:|mailto:|\/)/i;

/** Inline spans, innermost-first so `**a `b`**` still finds the code span. */
const INLINE = [
  { pattern: /`([^`]+)`/, render: (text: string, key: string) => <code key={key} className="rounded bg-surface-sunken px-1 py-0.5 font-mono text-[0.85em]">{text}</code> },
  { pattern: /\*\*([^*]+)\*\*/, render: (text: string, key: string) => <strong key={key} className="font-semibold">{text}</strong> },
  { pattern: /\*([^*]+)\*/, render: (text: string, key: string) => <em key={key}>{text}</em> },
] as const;
...
const HEADING_CLASS = ['text-xl', 'text-lg', 'text-base'] as const;
  ...
  while (index < lines.length) {
    ...
    if (line.trimStart().startsWith('```')) {
      ...
      blocks.push(
        <pre
          key={`b${start}`}
          className="overflow-x-auto rounded-md border border-line bg-surface-sunken p-3 font-mono text-xs text-ink"
        >
          <code>{body.join('\n')}</code>
        </pre>
      );
    ...
    if (heading) {
      ...
      blocks.push(
        <Tag key={key()} className={`mt-5 font-semibold text-ink ${HEADING_CLASS[level - 1]}`}>
    ...
    if (/^\s*>\s?/.test(line)) {
      ...
      blocks.push(
        <blockquote
          key={`b${start}`}
          className="border-l-2 border-brand-300 pl-3 text-sm italic text-ink-muted"
        >
    ...
    if (listPattern) {
      ...
      blocks.push(
        <Tag
          key={`b${start}`}
          className={`ml-5 space-y-1 text-sm text-ink ${ordered ? 'list-decimal' : 'list-disc'}`}
        >
          {items.map((item, offset) => (
            <li key={`b${start}-${offset}`}>{renderInline(item, `b${start}-${offset}i`)}</li>
          ))}
        </Tag>
      );
      continue;
    ...
    blocks.push(
      <p key={`b${start}`} className="text-sm leading-relaxed text-ink">
