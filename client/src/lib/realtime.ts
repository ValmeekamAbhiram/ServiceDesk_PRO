...
export interface RealtimeHandlers {
  onTicketEvent?: (event: ServerEvent, payload: TicketEventPayload) => void;
...
const TICKET_EVENTS: ServerEvent[] = [
...
function status(next: RealtimeStatus): void {
 ...
 */
export function connectRealtime(next: RealtimeHandlers): void {
  handlers = next;
  if (socket) {
  ...
  socket.on('connect', () => {
    ...
    for (const id of watched) socket?.emit(ClientEvent.WATCH_TICKET, id);
  });
  ...
  for (const event of TICKET_EVENTS) {
    socket.on(event, (payload: TicketEventPayload) => {
      handlers.onTicketEvent?.(event, payload);
    });
  }

  socket.on(ServerEvent.NOTIFICATION, (payload: NotificationDto) => {
 ...
 */
export function watchTicket(id: string): void {
  watched.add(id);
  socket?.emit(ClientEvent.WATCH_TICKET, id);
}

export function unwatchTicket(id: string): void {
  watched.delete(id);
