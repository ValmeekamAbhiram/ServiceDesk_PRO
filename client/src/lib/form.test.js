...
describe('applyServerErrors', () => {
    ...
    it('falls back to the form-level alert when a message has no field to land on', () => {
        const { form, setError } = formStub();
        const result = applyServerErrors(validationError([{ path: 'assetId', message: 'That asset is retired.' }]), form, FIELDS);
    ...
    it('routes a nested path to the control that owns it', () => {
        const { form, setError } = formStub();
        const result = applyServerErrors(validationError([{ path: 'categoryId.name', message: 'Unknown category.' }]), form, FIELDS);
        expect(result).toBeNull();
    ...
    it('does not treat a longer field name as a nested path', () => {
        const { form, setError } = formStub();
        const result = applyServerErrors(validationError([{ path: 'categoryIdOther', message: 'Nope.' }]), form, FIELDS);
        expect(result).toBe('Please check the highlighted fields.');
        expect(setError).not.toHaveBeenCalled();
    });
    it('shows a non-validation error message as it came from the server', () => {
        ...
        expect(applyServerErrors(error, form, FIELDS)).toBe('This ticket was changed by someone else while you were editing.');
        expect(setError).not.toHaveBeenCalled();
    });
