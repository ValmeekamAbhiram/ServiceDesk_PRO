...
export function useCategories(includeInactive = false) {
    return useQuery({
        ...
        queryFn: () => api.get('/categories', includeInactive ? { includeInactive: true } : {}),
...
export function useAssignees(enabled = true) {
    return useQuery({
        ...
        queryFn: () => api.get('/users/assignees'),
        staleTime: REFERENCE_STALE_MS,
        enabled,
    });
}
export function useCreateCategory() {
    return useMutation({
        meta: { silent: true },
        mutationFn: (input) => api.post('/categories', input),
...
export function useUpdateCategory(id) {
    return useMutation({
        meta: { silent: true },
        mutationFn: (input) => api.patch(`/categories/${id}`, input),
        onSuccess: () => {
