...
export function useUpdateSlaPolicy() {
    return useMutation({
        meta: { silent: true },
        mutationFn: (input) => api.patch('/sla-policy', input),
        onSuccess: (policy) => {
...
export function useUpdateSettings() {
    return useMutation({
        meta: { silent: true },
        mutationFn: (input) => api.patch('/settings', input),
        onSuccess: (settings) => {
 ...
 */
export function useDemoClock(enabled) {
    return useQuery({
        queryKey: keys.demoClock,
        queryFn: () => api.get('/demo/clock'),
        enabled,
        staleTime: 0,
    });
}
