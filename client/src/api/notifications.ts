...
import type { NotificationListDto } from '@shared/types';
import { api } from '@/lib/api';
import { keys, queryClient } from '@/lib/query';

export type NotificationQuery = { page?: number; limit?: number; unreadOnly?: boolean };

export function useNotifications(query: NotificationQuery) {
  return useQuery({
    queryKey: keys.notifications.list(query),
    queryFn: () =>
      api.get<NotificationListDto>('/notifications', {
        page: query.page,
        limit: query.limit,
...
export function useMarkNotificationRead(query: NotificationQuery) {
  return useMutation({
    meta: { silent: true },
    mutationFn: (id: string) =>
      api.post<NotificationListDto>(`/notifications/${id}/read`, undefined, {
        page: query.page,
        limit: query.limit,
        unreadOnly: query.unreadOnly,
      }),
...
export function useMarkAllNotificationsRead(query: NotificationQuery) {
  return useMutation({
    meta: { silent: true },
    mutationFn: () =>
      api.post<NotificationListDto>('/notifications/read-all', undefined, {
        page: query.page,
        limit: query.limit,
        unreadOnly: query.unreadOnly,
      }),
