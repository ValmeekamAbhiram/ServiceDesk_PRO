...
export default function AssetDetail() {
  ...
  if (asset.isLoading) {
    return (
      <div className="space-y-5" aria-label="Loading asset">
  ...
  if (asset.isError || !asset.data) {
    return (
      <EmptyState
        ...
        message="It may have been removed, or it may not be one you can see."
        action={
      ...
      <header className="rounded-card border border-line bg-surface px-4 py-4 shadow-card sm:px-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          ...
          <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:shrink-0">
            ...
            {canManage && (
              <Link to={`/assets/${data.id}/edit`} className="btn btn-ghost btn-sm flex-1 sm:flex-none">
          ...
          <CardHeader
            ...
            subtitle={
              data.openTicketCount > 0
                ...
                : `${data.ticketCount} in total`
            }
            action={
              data.ticketCount > 0 ? (
                <Link to={`/tickets?assetId=${data.id}`} className="link text-xs">
                  See all
                </Link>
              ) : undefined
            }
          ...
          <CardBody className="p-0">
            {tickets.isLoading ? (
              ...
              <EmptyState
                ...
                message="Linked tickets could not be loaded. The asset details are still available."
                className="py-9"
                action={
              ...
              <EmptyState
                ...
                className="py-9"
                action={
                  <Link to={`/tickets/new?assetId=${data.id}`} className="btn btn-secondary btn-sm">
                    <Plus className="h-4 w-4" aria-hidden="true" />
                    Raise a ticket
                  </Link>
                }
