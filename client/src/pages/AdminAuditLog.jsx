...
const stamp = (iso) => new Date(iso).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' });
export default function AdminAuditLog() {
    ...
    return (<div className="space-y-4">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-2xs font-medium text-ink-muted">
            Action
            <Select className="mt-1 w-auto" value={params.get('action') ?? ''} onChange={(event) => setParam('action', event.target.value)}>
              <option value="">Any action</option>
              {Object.values(AuditAction).map((action) => (<option key={action} value={action}>
          ...
          <label className="text-2xs font-medium text-ink-muted">
            Subject
            <Select className="mt-1 w-auto" value={params.get('entityType') ?? ''} onChange={(event) => setParam('entityType', event.target.value)}>
              <option value="">Anything</option>
              {Object.values(AuditEntity).map((entity) => (<option key={entity} value={entity}>
          ...
          * so picking the same date twice returns that day rather than nothing. */}
          <label className="text-2xs font-medium text-ink-muted">
            From
            <Input type="date" className="mt-1 w-auto" value={params.get('from') ?? ''} onChange={(event) => setParam('from', event.target.value)}/>
          </label>
          <label className="text-2xs font-medium text-ink-muted">
            To
            <Input type="date" className="mt-1 w-auto" value={params.get('to') ?? ''} onChange={(event) => setParam('to', event.target.value)}/>
          </label>
      ...
      <Card>
        {entries.isLoading ? (<div className="p-4">
            <table className="table">
              <tbody>
                <SkeletonRows rows={8} cols={4}/>
              </tbody>
            </table>
          </div>) : rows.length === 0 ? (<EmptyState icon={<ScrollText className="h-5 w-5" aria-hidden="true"/>} title={filtered ? 'Nothing matches those filters' : 'Nothing recorded yet'} message={filtered
                ? 'Widen the dates, or clear the filters to see the whole trail.'
                : 'Entries appear here as soon as anyone signs in or changes something.'}/>) : (<div className="overflow-x-auto">
            <table className="table">
              <thead>
                <tr>
              ...
              <tbody>
                {rows.map((entry) => (<AuditRow key={entry.id} entry={entry} now={now}/>))}
              </tbody>
            </table>
          </div>)}

        {entries.data && rows.length > 0 && (<Pagination meta={entries.data.meta} unit="record" onPageChange={(page) => setParam('page', String(page))}/>)}
      </Card>
    </div>);
}
/**
 ...
 * be compared at once.
 */
function AuditRow({ entry, now }) {
    const [open, setOpen] = useState(false);
    const action = AUDIT_ACTION_META[entry.action] ?? { label: entry.action, tone: 'neutral' };
    ...
    return (<tr className="align-top">
      ...
      <td className="whitespace-nowrap">
        ...
        {entry.automated && (<span className="text-2xs text-ink-subtle">Automatic</span>)}
      </td>
      <td className="whitespace-nowrap">
      ...
      <td>
        ...
        {changes.length > 0 && (<>
            <button type="button" className="mt-1 inline-flex items-center gap-1 text-2xs font-medium text-brand-700 hover:underline" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
            ...
            {open && (<dl className="mt-1.5 space-y-1 rounded-lg border border-line bg-surface-sunken px-3 py-2">
                {changes.map(([field, change]) => (<div key={field} className="flex flex-wrap items-baseline gap-x-2 text-2xs">
