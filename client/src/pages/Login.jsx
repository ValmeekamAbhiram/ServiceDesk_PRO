...
export default function Login() {
    ...
    return (<AuthLayout title="Sign in" subtitle="Use the account your IT team set up for you." footer={<p className="text-xs text-ink-muted">
          No account yet?{' '}
      ...
      <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)} noValidate>
        {failure && (<div role="alert" className="flex items-start gap-2 rounded-lg border border-danger-border bg-danger-bg px-3 py-2 text-xs text-danger-fg">
            <AlertCircle className="mt-px h-4 w-4 shrink-0" aria-hidden="true"/>
            <span>{failure}</span>
          </div>)}

        <Field label="Email address" htmlFor="email" error={form.formState.errors.email?.message} required>
          <Input id="email" type="email" autoComplete="email" autoFocus invalid={Boolean(form.formState.errors.email)} aria-describedby={form.formState.errors.email ? 'email-error' : undefined} {...form.register('email')}/>
        </Field>

        <Field label="Password" htmlFor="password" error={form.formState.errors.password?.message} required>
          <Input id="password" type="password" autoComplete="current-password" invalid={Boolean(form.formState.errors.password)} aria-describedby={form.formState.errors.password ? 'password-error' : undefined} {...form.register('password')}/>
        </Field>
