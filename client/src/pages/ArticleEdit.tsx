...
const FIELDS = ['title', 'summary', 'body', 'categoryId', 'tags'] as const;
...
export default function ArticleEdit() {
  ...
  useEffect(() => {
    ...
    if (!article || form.formState.isDirty) return;
    form.reset({
      title: article.title,
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      if (error instanceof ApiClientError && error.isVersionConflict) {
        ...
        setFormError(
          'Somebody else saved a change while you were editing. Your text is kept — saving again will replace their version.'
        );
        return;
      }
      ...
      <Card className="space-y-4 p-5">
        <Field label="Title" htmlFor="title" error={form.formState.errors.title?.message} required>
          <Input
            ...
            placeholder="Printer says “offline” after a restart"
          />
        </Field>
        ...
        <Field
          ...
          hint="Shown on the shelf and in search results. One or two sentences."
          required
        >
          <Textarea
            id="summary"
            ...
            invalid={Boolean(form.formState.errors.summary)}
          />
        </Field>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field
            ...
            hint={`Comma separated, up to ${MAX_TAGS}. Lower-cased on save.`}
          >
      ...
      <Card className="space-y-3 p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            <button
              type="button"
              className={preview ? 'chip' : 'chip border-brand-300 bg-brand-50 text-brand-700'}
            ...
            <button
              type="button"
              className={preview ? 'chip border-brand-300 bg-brand-50 text-brand-700' : 'chip'}
              aria-pressed={preview}
        ...
        {preview && (
          <div className="rounded-lg border border-line bg-surface-sunken p-4">
            {body.trim() === '' ? (
              <p className="text-xs text-ink-subtle">Nothing to preview yet.</p>
            ) : (
