...
export default function AssetList() {
    ...
    const toggleMulti = (key, value) => {
        ...
        next.delete(key);
        for (const kept of current.filter((entry) => entry !== value))
    ...
    return (<div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {canManage && (<Link to="/assets/new" className="btn btn-primary btn-sm">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-center gap-2">
          <form className="relative min-w-[14rem] flex-1" onSubmit={(event) => {
            ...
            <Input key={params.get('q') ?? ''} name="q" defaultValue={params.get('q') ?? ''} placeholder="Tag, name, serial number…" className="pl-8" aria-label="Search assets"/>
          </form>
          <Select aria-label="Warranty" value={params.get('warrantyWithinDays') ?? ''} onChange={(event) => setParam('warrantyWithinDays', event.target.value)} className="w-auto">
            {WARRANTY_WINDOWS.map((window) => (<option key={window.value} value={window.value}>
                {window.label}
              </option>))}
          </Select>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {Object.values(AssetStatus).map((status) => {
            ...
            return (<button key={status} type="button" className={on ? 'chip border-brand-300 bg-brand-50 text-brand-700' : 'chip'} aria-pressed={on} onClick={() => toggleMulti('status', status)}>
          ...
          {Object.values(AssetType).map((type) => {
            ...
            return (<button key={type} type="button" className={on ? 'chip border-brand-300 bg-brand-50 text-brand-700' : 'chip'} aria-pressed={on} onClick={() => toggleMulti('type', type)}>
      ...
      <Card>
        {assets.isLoading ? (<div className="p-4">
            <SkeletonRows rows={6} cols={5}/>
          </div>) : (assets.data?.items.length ?? 0) === 0 ? (<EmptyState icon={<ShieldAlert className="h-5 w-5" aria-hidden="true"/>} title={filtered ? 'Nothing matches those filters' : 'No assets yet'} message={filtered
                ? 'Widen the search, or clear the filters to see everything you can.'
                : 'Once hardware is registered here it can be linked to tickets.'}/>) : (<div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {(assets.data?.items ?? []).map((asset) => (<tr key={asset.id}>
                    <td>
                    ...
                    <td className="tabular text-right text-xs text-ink-muted">
                      {asset.openTicketCount > 0 ? (<span className="font-semibold text-ink">{asset.openTicketCount} open</span>) : (asset.ticketCount)}
                    </td>
                  </tr>))}
              </tbody>
            </table>
          </div>)}
        {assets.data && (<Pagination meta={assets.data.meta} unit="asset" onPageChange={(page) => setParam('page', String(page))}/>)}
      </Card>
    </div>);
}
