...
import { useTickets, type TicketQuery } from '@/api/tickets';
...
function readQuery(params: URLSearchParams): TicketQuery {
  ...
  return {
    ...
    status: list<TicketStatus>('status'),
    ...
    scope: (params.get('scope') as TicketQuery['scope']) ?? undefined,
    breached: params.get('breached') === 'true' ? true : undefined,
    sortBy: (params.get('sortBy') as TicketQuery['sortBy']) ?? 'createdAt',
...
export default function TicketList() {
  ...
  const toggleMulti = (key: 'status' | 'priority', value: string) => {
    const next = new URLSearchParams(params);
    ...
    const after = current.includes(value)
      ...
      : [...current, value];
    next.delete(key);
    for (const item of after) next.append(key, item);
    next.delete('page');
    setParams(next, { replace: true });
  };
    ...
    <div className="space-y-4">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[12rem] flex-1">
            ...
            <Input
              ...
              placeholder="Search number, subject or description…"
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter') setParam('q', event.currentTarget.value.trim() || null);
              }}
            />
          </div>
          ...
          <Select
            ...
            onChange={(event) => setParam('scope', event.target.value === 'all' ? null : event.target.value)}
            className="w-auto"
          >
            {SCOPES.map((scope) => (
        ...
        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
          ...
          <button
            type="button"
            aria-pressed={query.breached ?? false}
            className={query.breached ? 'chip border-danger-border bg-danger-bg text-danger-fg' : 'chip'}
            onClick={() => setParam('breached', query.breached ? null : 'true')}
          >
            Breached
          </button>
        </div>
      ...
      <Card>
        {list.isLoading ? (
          ...
          <EmptyState
            title={active > 0 ? 'No tickets match these filters' : 'No tickets yet'}
            message={
              active > 0
                ? 'Try widening the search, or clear the filters to see everything you have access to.'
                : 'When somebody raises a ticket it will appear here.'
            }
            action={
              active > 0 ? (
                <button
