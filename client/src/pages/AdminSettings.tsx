...
function SettingsForm({ settings }: { settings: SettingsDto }) {
  ...
  const dirty =
    organizationName.trim() !== settings.organizationName ||
    supportEmail.trim() !== settings.supportEmail;

  const save = async (patch: Parameters<typeof update.mutateAsync>[0], done: string) => {
    setError(null);
    try {
  ...
  return (
    <Card>
      <CardHeader
        title="Organisation"
        subtitle="The name shown in the header, and the address the desk is reached at."
      />
      <CardBody className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field
            ...
            hint="Shown under the product name in the sidebar."
          >
            <Input
              id="organizationName"
          ...
          <Field
            ...
            hint="Offered in the sidebar footer as the address to write to."
          >
            <Input
              id="supportEmail"
              type="email"
        ...
        <div className="flex justify-end">
          <Button
            ...
            onClick={() =>
              void save(
                { organizationName: organizationName.trim(), supportEmail: supportEmail.trim() },
                'Saved.'
              )
            }
          >
        ...
        <div className="space-y-4 border-t border-line pt-4">
          <Toggle
            id="aiSuggestions"
            label="Offline ticket suggestions"
            description="Suggests a category and priority from the words in a title. It never assigns, never changes a ticket on its own, and a technician can ignore it."
            ...
            note={
              settings.aiSuggestionsRequested && !settings.aiSuggestionsEnabled
            ...
            onChange={(next) =>
              void save(
                { aiSuggestionsEnabled: next },
                next ? 'Suggestions on.' : 'Suggestions off.'
              )
            }
          />
          ...
          <Toggle
            id="demoMode"
            label="Demo mode"
            description="Unlocks the Time Machine below, which moves the clock the SLA engine reads. Intended for a demonstration, never for a live desk."
            checked={settings.demoModeRequested}
            note={
              settings.demoModeRequested && !settings.demoMode
            ...
            onChange={(next) =>
              void save(
                { demoModeRequested: next },
                next ? 'Demo mode on.' : 'Demo mode off. The clock is back to real time.'
              )
            }
          />
        </div>
 ...
 */
function TimeMachine({ available }: { available: boolean }) {
  ...
  const move = async (minutes: number) => {
    try {
  ...
  return (
    <Card className="border-warning-border">
      <CardHeader
        ...
        subtitle="Moves the clock the SLA engine reads, so a deadline can be crossed in front of an audience instead of in four hours' time. No stored timestamp is rewritten — a ticket raised ten minutes ago still says ten minutes ago."
      />
      <CardBody className="space-y-4">
            ...
            <div>
              ...
              <dd>
                ...
                {state.mode === ClockMode.REAL && (
                  <span className="ml-1.5 text-2xs text-ink-subtle">real clock</span>
                )}
              </dd>
            </div>
          </dl>
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-line pt-4">
          {JUMPS.map((jump) => (
            <Button
              ...
              variant="secondary"
              disabled={busy}
              onClick={() => void move(jump.minutes)}
            >
              {jump.label}
            </Button>
          ))}
          ...
          <Button
            ...
            onClick={() => {
              void reset
                ...
                .catch((error: unknown) =>
                  toast.error(
                    error instanceof ApiClientError ? error.message : 'The clock did not move.'
                  )
                );
            }}
          >
