...
import { useSlaPolicy, useUpdateSlaPolicy, type SlaPolicyPatch } from '@/api/admin';
...
const FIELDS: readonly FieldPath<Values>[] = [
  'timezone',
  'startTime',
  'endTime',
  'workingDays',
  ...
  ...PRIORITIES.flatMap(
    (priority) =>
      [`targets.${priority}.responseMinutes`, `targets.${priority}.resolutionMinutes`] as const
  ),
];
  ...
  return (
    <div className="space-y-4">
      ...
      {policy.isLoading ? (
        ...
        <PolicyForm key={policy.data.updatedAt} policy={policy.data} />
      ) : (
        <Card className="p-5 text-sm text-ink-subtle">The policy could not be loaded.</Card>
      )}
    </div>
  );
}

function PolicyForm({ policy }: { policy: SlaPolicyDto }) {
    ...
    <form className="space-y-4" onSubmit={submit} noValidate>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="Applies immediately, to every open ticket. Deadlines are counted in business time, so a ticket raised at 17:55 on Friday has five minutes of Friday and then waits for Monday."
        />
        <CardBody className="space-y-4">
          ...
          <Field
            label="Warn at"
            htmlFor="atRiskThresholdPercent"
            error={errors.atRiskThresholdPercent?.message}
            hint="Percent of the budget consumed before a ticket is flagged at risk. Applies immediately."
            className="max-w-[12rem]"
            required
          >
            <Input
              id="atRiskThresholdPercent"
              type="number"
              min={1}
              max={99}
              {...form.register('atRiskThresholdPercent')}
              invalid={Boolean(errors.atRiskThresholdPercent)}
            />
          </Field>
        </CardBody>
      </Card>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="In business minutes. These apply to tickets raised from now on — a ticket's deadline is fixed when it is raised, so shortening a budget today cannot breach yesterday's tickets."
        />
            ...
            <tbody>
              {PRIORITIES.map((priority) => (
                <BudgetRow
                  ...
                  priority={priority}
                  form={form}
                  current={policy.targets[priority]}
                />
              ))}
            </tbody>
          </table>
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
        {isDirty && (
          <p className="mr-auto text-2xs text-ink-subtle">
...
function BudgetRow({
  ...
  current: { responseMinutes: number; resolutionMinutes: number };
}) {
    ...
    <tr>
      ...
      <td>
        <Input
          type="number"
          min={1}
          aria-label={`First response minutes for ${priority.toLowerCase()} priority`}
          className="w-28"
          {...form.register(`targets.${priority}.responseMinutes`)}
          invalid={Boolean(errors?.responseMinutes)}
        />
        {errors?.responseMinutes && (
      ...
      <td>
        <Input
          type="number"
          min={1}
          aria-label={`Resolution minutes for ${priority.toLowerCase()} priority`}
          className="w-28"
          {...form.register(`targets.${priority}.resolutionMinutes`)}
          invalid={Boolean(errors?.resolutionMinutes)}
        />
