...
const FIELDS = ['name', 'description', 'color', 'defaultPriority', 'keywords', 'sortOrder'];
const BLANK = {
    name: '',
    description: '',
    color: '#2563eb',
    ...
    return (<div className="space-y-4">
      ...
      <Card>
        {categories.isLoading ? (<div className="p-4">
          ...
          </div>) : rows.length === 0 ? (<EmptyState icon={<FolderTree className="h-5 w-5" aria-hidden="true"/>} title="No categories yet" message="A ticket has to be filed under something, so add at least one." action={<Button size="sm" onClick={() => setEditing('new')}>
                Add the first one
            ...
            <table className="table">
              ...
              <tbody>
                {rows.map((category) => (<CategoryRow key={category.id} category={category} onEdit={() => setEditing(category)}/>))}
              </tbody>
            </table>
          </div>)}
      </Card>
...
function CategoryRow({ category, onEdit }) {
    ...
    return (<tr className={category.active ? undefined : 'opacity-60'}>
      <td>
        <div className="flex items-start gap-2">
          {/* The only place a stored colour is interpolated into a style attribute, which
          * is why `#rrggbb` is enforced at both ends. */}
          <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: category.color }} aria-hidden="true"/>
          <div className="min-w-0">
            <p className="flex items-center gap-1.5 font-medium text-ink">
              {category.name}
              {!category.active && <Badge tone="neutral">Retired</Badge>}
            </p>
            {category.description && (<p className="max-w-md text-2xs leading-relaxed text-ink-subtle">{category.description}</p>)}
          </div>
        </div>
      </td>
      <td><PriorityBadge priority={category.defaultPriority}/></td>
      <td>
        {category.keywords.length === 0 ? (<span className="text-2xs text-ink-subtle">None — matches the name only</span>) : (<div className="flex max-w-sm flex-wrap gap-1">
            {category.keywords.slice(0, 6).map((word) => (<span key={word} className="chip">{word}</span>))}
            {category.keywords.length > 6 && (<span className="chip">+{category.keywords.length - 6}</span>)}
          </div>)}
      </td>
...
function CategoryModal({ category, onClose }) {
    ...
    return (<Modal open onClose={onClose} title={category ? `Edit ${category.name}` : 'New category'} description="Renaming a category updates it everywhere it is shown, including on tickets already filed under it." footer={<>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button type="submit" form="category-form" loading={create.isPending || update.isPending}>
      ...
      <form id="category-form" className="space-y-4" onSubmit={submit} noValidate>
          ...
          * `watch` and writes back through `setValue`. */}
          <Field label="Colour" htmlFor="color" error={form.formState.errors.color?.message}>
            <div className="flex items-center gap-2">
              <input id="color" type="color" className="h-9 w-12 shrink-0 cursor-pointer rounded border border-line bg-surface p-1" {...form.register('color')}/>
              <Input aria-label="Hex colour" className="font-mono text-xs" value={form.watch('color')} invalid={Boolean(form.formState.errors.color)} onChange={(event) => form.setValue('color', event.target.value, { shouldValidate: true, shouldDirty: true })}/>
            </div>
          </Field>
          <Field label="Starting priority" htmlFor="defaultPriority" error={form.formState.errors.defaultPriority?.message} hint="A requester can still change it.">
            <Select id="defaultPriority" {...form.register('defaultPriority')}>
              {Object.values(Priority).map((priority) => (<option key={priority} value={priority}>{PRIORITY_META[priority].label}</option>))}
            </Select>
          </Field>
        ...
        <Field label="Classifier keywords" htmlFor="keywords" error={form.formState.errors.keywords?.message} hint={`Comma separated, up to ${MAX_KEYWORDS}, lower-cased on save. These only feed the suggestion panel — they never file a ticket on their own.`}>
