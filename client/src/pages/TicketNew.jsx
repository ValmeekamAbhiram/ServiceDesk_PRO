...
const FIELDS = ['title', 'description', 'categoryId', 'priority', 'assetId'];
/* ─────────────────────────────── suggestions ─────────────────────────────── */
function SuggestionPanel({ title, description, onApply, }) {
    ...
    const ask = () => {
        setNote(null);
        suggest.mutate({ title: title.trim(), description: description.trim() || undefined }, {
            onSuccess: (data) => setResult(data),
            onError: (error) => {
                setResult(null);
                setNote(error instanceof Error && 'status' in error && error.status === 503
    ...
    return (<Card className="border-violet-border bg-violet-bg">
      <CardHeader title={<span className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-violet-fg" aria-hidden="true"/>
            Suggested classification
          </span>} subtitle="A guess from the words you typed. It fills the form in; it never files the ticket."/>
...
function SuggestionBody({ suggestion, onApply, }) {
    ...
    return (<div className="space-y-3 border-t border-line pt-3">
      ...
      {suggestion.categoryId && (<Button variant="secondary" size="sm" onClick={onApply}>
          Use these values
        </Button>)}
      {suggestion.relatedArticles.length > 0 && (<div className="space-y-1.5 border-t border-line pt-3">
          <p className="text-xs font-medium text-ink-muted">This may already be answered</p>
          <ul className="space-y-1.5">
            {suggestion.relatedArticles.map((article) => (<li key={article.id}>
                {/* Opens in a new tab on purpose: reading an article should not throw
                    away a half-written ticket. */}
                <Link to={`/knowledge/${article.id}`} target="_blank" rel="noreferrer" className="link text-xs font-medium">
                  {article.title}
                </Link>
        ...
        <Card>
          <CardBody>
            <form className="space-y-4" onSubmit={submit} noValidate>
              {banner && (<p role="alert" className="rounded-lg border border-danger-border bg-danger-bg px-3 py-2 text-xs text-danger-fg">
                  {banner}
                </p>)}

              <Field label="Title" htmlFor="title" required error={form.formState.errors.title?.message}>
                <Input id="title" autoFocus placeholder="Laptop will not connect to the office Wi-Fi" invalid={Boolean(form.formState.errors.title)} aria-describedby={form.formState.errors.title ? 'title-error' : undefined} {...form.register('title')}/>
              </Field>

              <Field label="What is happening?" htmlFor="description" required hint="When it started, what you were doing, any error message." error={form.formState.errors.description?.message}>
                <Textarea id="description" rows={7} invalid={Boolean(form.formState.errors.description)} aria-describedby={form.formState.errors.description ? 'description-error' : undefined} {...form.register('description')}/>
              </Field>

              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Category" htmlFor="categoryId" required error={form.formState.errors.categoryId?.message}>
                  <Select id="categoryId" invalid={Boolean(form.formState.errors.categoryId)} aria-describedby={form.formState.errors.categoryId ? 'categoryId-error' : undefined} {...form.register('categoryId')}>
                    <option value="">Choose one…</option>
                    {activeCategories.map((category) => (<option key={category.id} value={category.id}>
                        {category.name}
                      </option>))}
                  </Select>
                </Field>

                <Field label="Priority" htmlFor="priority" hint="Leave blank to use the category default." error={form.formState.errors.priority?.message}>
                  <Select id="priority" {...form.register('priority')}>
                    <option value="">Category default</option>
                    {Object.values(Priority).map((value) => (<option key={value} value={value}>
              ...
              <Field label="Affected device" htmlFor="assetId" hint="Optional. Linking one shows its history to whoever picks this up." error={form.formState.errors.assetId?.message}>
                <Select id="assetId" {...form.register('assetId')}>
                  <option value="">Not about a specific device</option>
                  {(assets.data?.items ?? []).map((asset) => (<option key={asset.id} value={asset.id}>
                      {asset.tag} — {asset.name}
                    </option>))}
                </Select>
              </Field>

              <Field label="Attachments" htmlFor="files" hint={`Up to ${MAX_FILES} files. Screenshots help more than descriptions of screenshots.`}>
