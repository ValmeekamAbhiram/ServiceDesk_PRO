...
const SETTLED: TicketStatus[] = [TicketStatus.CLOSED];
 ...
 */
function reportFailure(error: unknown, refresh: () => void): void {
...
function StatusPanel({ ticket, refresh }: { ticket: TicketDto; refresh: () => void }) {
  ...
  const apply = async (status: TicketStatus, resolutionNote?: string) => {
    try {
  ...
  return (
    <div className="space-y-2">
      <Select
        id="status"
        value={target}
        onChange={(event) => setTarget(event.target.value as TicketStatus)}
      >
      ...
      <Button
        ...
        fullWidth
        disabled={!target}
        loading={change.isPending && !confirming}
        onClick={() => {
          if (!target) return;
          if (needsNote) setConfirming(true);
      ...
      <Modal
        open={confirming && needsNote}
        onClose={() => setConfirming(false)}
        title="Resolve this ticket"
        description="What fixed it? The requester reads this, and it is what makes the ticket useful to the next person with the same problem."
        size="md"
        footer={
          <>
            ...
            <Button
              ...
              onClick={() => void apply(TicketStatus.RESOLVED, note.trim())}
            >
              Mark resolved
            </Button>
          </>
      ...
      >
        <Field label="Resolution" htmlFor="resolutionNote" required>
          <Textarea
            ...
            placeholder="Replaced the docking station; the port on the old one was dead."
          />
        </Field>
      </Modal>
    </div>
...
function AssignPanel({ ticket, refresh }: { ticket: TicketDto; refresh: () => void }) {
  const assign = useAssignTicket(ticket.id);
  const assignees = useAssignees();
  const me = useAuthStore((state) => state.user);

  const set = async (assigneeId: string | null) => {
    try {
  ...
  return (
    <div className="space-y-2">
      ...
      {canTakeIt && ticket.assignee?.id !== me?.id && (
        <Button
          variant="secondary"
          size="sm"
          fullWidth
          loading={assign.isPending}
          onClick={() => void set(me?.id ?? null)}
        >
...
function PropertyRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[minmax(6.5rem,0.8fr)_minmax(0,1.2fr)] items-start gap-3 py-2">
...
function Properties({ ticket, now }: { ticket: TicketDto; now: number }) {
  return (
    <dl className="divide-y divide-line">
      ...
      <PropertyRow label="Device">
        {ticket.asset ? (
          <Link to={`/assets/${ticket.asset.id}`} className="link inline-flex items-center gap-1">
  ...
  if (ticket.isError || !ticket.data) {
    ...
    return (
      <EmptyState
        ...
        message="It may have been deleted, or it may belong to someone else."
        action={
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-5">
          <Card>
            <CardHeader
              title="The problem"
              subtitle={PRIORITY_META[data.priority].description ?? undefined}
            />
          ...
          <Card>
            <CardHeader
              ...
              subtitle={`${data.commentCount} ${data.commentCount === 1 ? 'reply' : 'replies'}`}
            />
            <CardBody>
              {comments.isError ? (
                <EmptyState
                  ...
                  message="The replies could not be loaded. The rest of the ticket is still available."
                  className="py-8"
                  action={
                ...
                <CommentThread
                  ...
                  canReply={!SETTLED.includes(data.status)}
                />
              )}
            </CardBody>
          </Card>
          ...
          <Card className={data.sla.breached ? 'border-danger-border' : undefined}>
            <CardHeader
              title={
                <span className="flex items-center gap-2">
                  <Activity
                    className={data.sla.breached ? 'h-4 w-4 text-danger-fg' : 'h-4 w-4 text-brand-600'}
                    aria-hidden="true"
                  />
                  Service level
                </span>
              }
              subtitle={data.sla.policyName}
              action={
                data.sla.breached ? (
                  <span className="text-xs font-semibold text-danger-fg">Attention required</span>
                ) : undefined
              }
            />
