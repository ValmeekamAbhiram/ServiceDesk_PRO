...
export default function AdminUsers() {
    ...
    const apply = async () => {
        if (!pending)
            return;
        try {
            await update.mutateAsync(pending.kind === 'role' ? { role: pending.role } : { status: pending.status });
            toast.success(pending.kind === 'role'
    ...
    return (<div className="space-y-4">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-center gap-2">
          <form className="relative min-w-[14rem] flex-1" onSubmit={(event) => {
            ...
            <Input key={params.get('q') ?? ''} name="q" defaultValue={params.get('q') ?? ''} placeholder="Name or email…" className="pl-8" aria-label="Search people"/>
          </form>
          <Select aria-label="Sort by" value={query.sortBy} onChange={(event) => setParam('sortBy', event.target.value)} className="w-auto">
            {SORTS.map((sort) => (<option key={sort.value} value={sort.value}>
                {sort.label}
              </option>))}
          </Select>
          {filtered && (<button type="button" className="btn btn-ghost btn-sm" onClick={() => setParams({})}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          {Object.values(Role).map((role) => {
            ...
            return (<button key={role} type="button" className={on ? 'chip border-brand-300 bg-brand-50 text-brand-700' : 'chip'} aria-pressed={on} onClick={() => setParam('role', on ? null : role)}>
          ...
          {Object.values(UserStatus).map((status) => {
            ...
            return (<button key={status} type="button" className={on ? 'chip border-brand-300 bg-brand-50 text-brand-700' : 'chip'} aria-pressed={on} onClick={() => setParam('status', on ? null : status)}>
      ...
      <Card>
        {users.isLoading ? (<div className="p-4">
          ...
          </div>) : rows.length === 0 ? (<EmptyState icon={<Users className="h-5 w-5" aria-hidden="true"/>} title={filtered ? 'Nobody matches those filters' : 'Nobody has registered yet'} message={filtered ? 'Clear the filters to see the whole directory.' : undefined}/>) : (<div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                ...
                return (<tr key={user.id}>
                      ...
                      * everybody out. */}
                          <Select aria-label={`Role for ${user.name}`} className="w-auto" disabled={isSelf} title={isSelf ? 'Ask another administrator to change your own role.' : undefined} value={user.role} onChange={(event) => setPending({ user, kind: 'role', role: event.target.value })}>
                            {Object.values(Role).map((role) => (<option key={role} value={role}>
                                {ROLE_META[role].label}
                              </option>))}
                          </Select>
                          <Button size="sm" variant={user.status === UserStatus.ACTIVE ? 'secondary' : 'primary'} disabled={isSelf} title={isSelf ? 'You cannot deactivate your own account.' : undefined} onClick={() => setPending({
                        user,
                        kind: 'status',
        ...
        {users.data && (<Pagination meta={users.data.meta} unit="user" onPageChange={(page) => setParam('page', String(page))}/>)}
      </Card>
      ...
      <Modal open={pending !== null} onClose={() => setPending(null)} title={pending?.kind === 'role' ? 'Change this person’s role?' : pending?.status === UserStatus.INACTIVE ? 'Deactivate this account?' : 'Reactivate this account?'} footer={<>
            <Button variant="ghost" onClick={() => setPending(null)}>
              Cancel
            </Button>
            <Button variant={pending?.kind === 'status' && pending.status === UserStatus.INACTIVE ? 'danger' : 'primary'} loading={update.isPending} onClick={() => void apply()}>
              Confirm
            </Button>
          </>}>
        {pending && (<div className="space-y-3 text-sm">
            ...
            {pending.kind === 'role' ? (<div className="space-y-2">
              ...
              </div>) : pending.status === UserStatus.INACTIVE ? (<p className="text-ink-muted">
              ...
              </p>) : (<p className="text-ink-muted">They will be able to sign in again with their existing password.</p>)}
          </div>)}
      </Modal>
    </div>);
}
