...
export default function ArticleList() {
    ...
    return (<div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {canWrite && (<Link to="/knowledge/new" className="btn btn-primary btn-sm">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-center gap-2">
          <form className="relative min-w-[14rem] flex-1" onSubmit={(event) => {
            ...
            <Input key={params.get('q') ?? ''} name="q" defaultValue={params.get('q') ?? ''} placeholder="Search titles, summaries, tags…" className="pl-8" aria-label="Search articles"/>
          </form>
          <Select aria-label="Category" value={params.get('categoryId') ?? ''} onChange={(event) => setParam('categoryId', event.target.value)} className="w-auto">
          ...
          * is worse than one that says why. */}
          <Select aria-label="Sort by" title={query.q ? 'Search results are ordered by relevance.' : undefined} disabled={Boolean(query.q)} value={query.sortBy} onChange={(event) => setParam('sortBy', event.target.value)} className="w-auto">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          ...
          * way out rather than listing every tag in the corpus. */}
          {query.tag && (<button type="button" className="chip border-brand-300 bg-brand-50 text-brand-700" onClick={() => setParam('tag', null)}>
      ...
      {articles.isLoading ? (<Card className="p-4">
          <SkeletonRows rows={4} cols={2}/>
        </Card>) : rows.length === 0 ? (<Card>
          <EmptyState icon={<BookOpen className="h-5 w-5" aria-hidden="true"/>} title={filtered ? 'Nothing matches that' : 'The shelf is empty'} message={filtered
                ...
                : canWrite
                    ...
                    : 'Nothing has been published yet. Raise a ticket and someone will help.'} action={canWrite && !filtered ? (<Link to="/knowledge/new" className="btn btn-primary btn-sm">
                  Write the first one
                </Link>) : undefined}/>
        </Card>) : (<>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {rows.map((article) => (<Link key={article.id} to={`/knowledge/${article.id}`} className="card flex flex-col gap-2 p-4 transition hover:border-brand-300 hover:shadow-sm">
          ...
          {articles.data && (<Card>
              <Pagination meta={articles.data.meta} unit="article" onPageChange={(page) => setParam('page', String(page))}/>
            </Card>)}
        </>)}
    </div>);
}
