...
export interface AccessTokenClaims extends JwtPayload {
  /** User id. */
  sub: string;
  /** Session id — the handle that makes logout and revocation effective. */
  sid: string;
...
export interface AccessTokenResult {
  token: string;
  /** Absolute expiry, so the caller can tell the client when to refresh. */
  expiresAt: Date;
...
export function signAccessToken(
  ...
  const token = jwt.sign(
    { sub: userId, sid: sessionId, typ: TokenType.ACCESS, iat: issuedAtSeconds },
    env.JWT_SECRET,
    options
  );
 ...
 */
export function verifyAccessToken(token: string, nowMs: number): AccessTokenClaims {
  ...
  if (typeof payload === 'string' || payload === null) throw new TokenInvalidError();
   ...
   */
  if (claims.typ !== TokenType.ACCESS) throw new TokenInvalidError('Wrong token type.');
