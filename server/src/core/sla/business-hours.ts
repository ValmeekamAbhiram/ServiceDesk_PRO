...
/**
 * Structurally identical to `BusinessHoursConfig` on the SLA policy document, but
 * declared here so `core/sla` never imports a Mongoose model. A policy document
 * can be passed to any of these functions directly.
 */
export interface BusinessHours {
  /** IANA zone name, e.g. `Asia/Kolkata`. */
  timezone: string;
  /** Opening bell, minutes from midnight. 540 = 09:00. */
  startMinute: number;
  /** Closing bell, minutes from midnight. 1080 = 18:00. */
  endMinute: number;
  /** 0 = Sunday … 6 = Saturday. */
...
export function normalizeBusinessHours(hours: BusinessHours): BusinessHours {
  ...
  const workingDays = Array.from(
    new Set((hours.workingDays ?? []).filter((day) => Number.isInteger(day) && day >= 0 && day <= 6))
  ).sort((a, b) => a - b);
...
function clampMinute(value: number, fallback: number, min: number, max: number): number {
  if (!Number.isFinite(value)) return fallback;
  return Math.min(Math.max(Math.floor(value), min), max);
}

/** Business minutes in one full working day. */
export function minutesPerBusinessDay(hours: BusinessHours): number {
...
export function minutesPerBusinessWeek(hours: BusinessHours): number {
  const safe = normalizeBusinessHours(hours);
...
/** A calendar date in the business timezone. Month is 1-based, as humans write it. */
 ...
 */
function instantAtZoned(date: CalendarDate, minuteOfDay: number, timezone: string): Date {
  ...
  if (corrected !== firstGuess) ms = wallMs - corrected;
  return new Date(ms);
}

/* ──────────────────────────── the working calendar ─────────────────────────── */

function calendarDateOf(instant: Date, timezone: string): CalendarDate {
  const wall = wallClockOf(instant, timezone);
...
/** The open/close instants of one calendar day, or null if the desk is shut. */
interface BusinessWindow {
  opensAt: number;
  closesAt: number;
}

function windowOn(date: CalendarDate, hours: BusinessHours): BusinessWindow | null {
  if (!hours.workingDays.includes(weekdayOf(date))) return null;
  return {
 ...
 */
export function nextBusinessStart(instant: Date, hours: BusinessHours): Date {
  const safe = normalizeBusinessHours(hours);
  const ms = instant.getTime();
...
export function addBusinessMinutes(from: Date, minutes: number, hours: BusinessHours): Date {
  if (!Number.isFinite(minutes) || minutes <= 0) return new Date(from.getTime());
  ...
  for (let scanned = 0; scanned < MAX_DAYS_SCANNED; scanned += 1) {
    ...
    if (window && cursorMs < window.closesAt) {
      ...
      if (remaining <= available) return new Date(position + remaining * MINUTE_MS);
      remaining -= available;
    }
    date = nextCalendarDay(date);
  ...
  throw new Error(
    `addBusinessMinutes: ${minutes} business minutes exceeds ${MAX_DAYS_SCANNED} days of the configured calendar.`
  );
}
 ...
 * frozen overnight.
 */
export function msUntilBusinessHours(instant: Date, hours: BusinessHours): number {
  if (isWithinBusinessHours(instant, hours)) return 0;
...
export function formatBusinessWindow(hours: BusinessHours): string {
  const safe = normalizeBusinessHours(hours);
  const label = (minute: number): string => {
 ...
 */
export function businessDateKey(instant: Date, hours: BusinessHours): string {
  const safe = normalizeBusinessHours(hours);
 ...
 */
export function recentBusinessDays(
  now: Date,
  days: number,
  hours: BusinessHours
): { keys: string[]; from: Date } {
  const safe = normalizeBusinessHours(hours);
