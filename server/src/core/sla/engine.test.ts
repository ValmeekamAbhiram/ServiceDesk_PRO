...
import type { BusinessHours } from '@/core/sla/business-hours';
import {
  budgetFor,
  emptyTarget,
  escalate,
...
const POLICY: SlaPolicySnapshot = {
...
function istLabel(date: Date | null): string {
  if (!date) return 'null';
...
describe('starting the clock', () => {
  ...
  it('falls back to the MEDIUM budget for a priority the policy omits', () => {
    const thin: SlaPolicySnapshot = {
      ...POLICY,
      targets: { [Priority.MEDIUM]: POLICY.targets[Priority.MEDIUM] } as SlaPolicySnapshot['targets'],
    };
    expect(budgetFor(thin, Priority.URGENT)).toEqual({
      responseMinutes: 240,
      resolutionMinutes: 1440,
    });
    expect(budgetFor({ ...thin, targets: {} as SlaPolicySnapshot['targets'] }, Priority.LOW)).toEqual(
      { responseMinutes: 0, resolutionMinutes: 0 }
    );
  });
});
...
describe('ON_TRACK → AT_RISK → BREACHED', () => {
  ...
  it('honours a policy that warns earlier', () => {
    const jumpy: SlaPolicySnapshot = { ...POLICY, atRiskThresholdPercent: 25 };
...
describe('evaluateTicketSla', () => {
  ...
  it('reports the policy name the deadlines were set under', () => {
    expect(evaluateTicketSla(sla, ist('2026-01-05T09:05'), POLICY).policyName).toBe(
      'Standard support policy'
    );
    // A ticket seeded before the field existed still gets a usable label.
    const nameless = { ...sla, policyName: '' };
    expect(evaluateTicketSla(nameless, ist('2026-01-05T09:05'), POLICY).policyName).toBe(
      'Standard support policy'
    );
  });
});
...
describe('worstSlaState', () => {
  it('picks the state that needs attention', () => {
    ...
    expect(worstSlaState([SlaState.ON_TRACK, SlaState.BREACHED, SlaState.AT_RISK])).toBe(
      SlaState.BREACHED
    );
  });
});
