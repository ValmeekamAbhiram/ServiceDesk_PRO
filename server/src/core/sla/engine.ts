...
/** Per-priority budgets, in business minutes. Mirrors `SlaTargetBudget`. */
export interface SlaBudget {
  responseMinutes: number;
  resolutionMinutes: number;
}

/** Exactly what a ticket persists per target. Mirrors `TicketSlaTarget`. */
export interface SlaTargetSnapshot {
  /**
   * When this particular clock started. Per target, not per ticket, because a
   * reopen restarts the resolution clock and leaves the response clock alone — after
   * that the two targets genuinely have different start instants, and a single
   * ticket-level value would have to be wrong for one of them.
   *
   * Stored so `repriceTarget` can recompute a deadline without being handed a start
   * instant by its caller. `ticket.createdAt` is the obvious thing to pass and the
   * wrong thing: Mongoose writes it from the real system clock, while the SLA is
   * computed from the injectable one, so under the demo clock they are months apart.
   */
  startedAt: Date | null;
...
/** Both targets plus the policy name as it was when they were set. */
export interface TicketSlaSnapshot {
  policyName: string;
  response: SlaTargetSnapshot;
  resolution: SlaTargetSnapshot;
}

/**
 * The policy, flattened. Structurally satisfied by an `SlaPolicyDoc`, so the SLA
 * service can pass the Mongoose document straight in without this module ever
 * importing a model.
 */
export interface SlaPolicySnapshot {
  ...
  targets: Record<Priority, SlaBudget>;
}

/** The derived shape a response needs. `toSlaTargetDto` just ISO-formats it. */
export interface SlaTargetView {
  state: SlaState;
  dueAt: Date | null;
  metAt: Date | null;
  /** Signed wall-clock milliseconds to the deadline. Negative once overdue. */
  remainingMs: number | null;
  /** Business minutes consumed as a percentage of the budget. Can exceed 100. */
  percentUsed: number;
  budgetMinutes: number;
}
 ...
 */
export const SLA_SEVERITY: Record<SlaState, number> = {
  [SlaState.MET]: 0,
  [SlaState.ON_TRACK]: 1,
  [SlaState.AT_RISK]: 2,
  [SlaState.BREACHED]: 3,
 ...
 */
export function budgetFor(policy: SlaPolicySnapshot, priority: Priority): SlaBudget {
  return (
...
export function startTarget(
  ...
  if (!Number.isFinite(budgetMinutes) || budgetMinutes <= 0) return emptyTarget();
  return {
    startedAt,
...
export function startTicketSla(
  createdAt: Date,
  priority: Priority,
 ...
 */
export function markTargetMet(target: SlaTargetSnapshot, at: Date): SlaTargetSnapshot {
  if (target.metAt) return target;
 ...
 */
function percentConsumed(
  target: SlaTargetSnapshot,
  reference: Date,
  hours: BusinessHours
): number {
  const { dueAt, budgetMinutes } = target;
  if (!dueAt || budgetMinutes <= 0) return 0;
...
export function projectTargetState(
  target: SlaTargetSnapshot,
  now: Date,
  policy: SlaPolicySnapshot
): SlaState {
 ...
 */
export function escalate(cached: SlaState, computed: SlaState): SlaState {
  if (isTerminalSlaState(cached)) return cached;
  if (isTerminalSlaState(computed)) return computed;
 ...
 */
export function sweepTicketSla(
  sla: TicketSlaSnapshot,
  now: Date,
  policy: SlaPolicySnapshot
): { response: SlaState; resolution: SlaState } | null {
  const response = escalate(sla.response.state, projectTargetState(sla.response, now, policy));
  const resolution = escalate(
    sla.resolution.state,
    projectTargetState(sla.resolution, now, policy)
  );
  if (response === sla.response.state && resolution === sla.resolution.state) return null;
...
export function repriceTarget(
  ...
  const carried: SlaTargetSnapshot = { ...fresh, metAt: existing.metAt };
 ...
 */
export function repriceTicketSla(
  sla: TicketSlaSnapshot,
  priority: Priority,
  now: Date,
  policy: SlaPolicySnapshot
 ...
 */
export function reopenTicketSla(
  sla: TicketSlaSnapshot,
  reopenedAt: Date,
  priority: Priority,
  policy: SlaPolicySnapshot
...
export function worstSlaState(states: readonly SlaState[]): SlaState {
  return states.reduce<SlaState>(
    (worst, state) => (SLA_SEVERITY[state] > SLA_SEVERITY[worst] ? state : worst),
    SlaState.MET
  );
}
