...
const PERMISSION_ORDER = new Map<Permission, number>(
  PERMISSIONS.map((permission, index) => [permission, index])
);

export function resolvePermissions(role: Role): Set<Permission> {
  return new Set<Permission>(ROLE_PERMISSIONS[role] ?? []);
}

/** Deterministic array form, for DTOs and audit diffs. */
export function permissionList(permissions: Iterable<Permission>): Permission[] {
  return Array.from(new Set(permissions)).sort(
    (a, b) => (PERMISSION_ORDER.get(a) ?? 999) - (PERMISSION_ORDER.get(b) ?? 999)
  );
}
