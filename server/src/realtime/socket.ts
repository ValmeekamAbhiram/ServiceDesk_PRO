...
/** Attached to each socket once the handshake succeeds. */
interface SocketState {
  /**
   * Everything an `ActorContext` needs except the clock, which is read per event
   * rather than captured: in demo mode the Time Machine swaps the clock instance,
   * and a connection opened before the swap must still see the new "now".
   */
  identity: Omit<ActorContext, 'clock'>;
  watching: Set<string>;
}

function actorOf(entry: SocketState): ActorContext {
 ...
 */
async function authorizeHandshake(socket: Socket): Promise<void> {
 ...
 */
async function watchTicket(socket: Socket, rawId: unknown): Promise<void> {
  const entry = state.get(socket);
  if (!entry) return;

  const id = typeof rawId === 'string' ? rawId.trim() : '';
  if (!OBJECT_ID.test(id)) return;
  if (entry.watching.has(id)) return;
...
async function unwatchTicket(socket: Socket, rawId: unknown): Promise<void> {
  const entry = state.get(socket);
  const id = typeof rawId === 'string' ? rawId.trim() : '';
  if (!entry || !entry.watching.delete(id)) return;
  await socket.leave(ticketRoom(id));
}

/** Rooms the server assigns. The client is never asked which it wants. */
async function joinAssignedRooms(socket: Socket, entry: SocketState): Promise<void> {
  await socket.join(userRoom(entry.identity.user.id));
  if (isStaff(actorOf(entry))) await socket.join(QUEUE_ROOM);
}
...
function onConnection(socket: Socket): void {
   ...
   */
  socket.on(ClientEvent.WATCH_TICKET, (id: unknown) => {
    void watchTicket(socket, id).catch((error: unknown) =>
      log.warn({ err: error }, 'Ticket watch failed.')
    );
  });

  socket.on(ClientEvent.UNWATCH_TICKET, (id: unknown) => {
    void unwatchTicket(socket, id).catch((error: unknown) =>
      log.warn({ err: error }, 'Ticket unwatch failed.')
    );
  });
...
export function attachSocketServer(httpServer: HttpServer): Server {
  ...
  const channel: Namespace = io.of(SOCKET_NAMESPACE);

  channel.use((socket, next) => {
    authorizeHandshake(socket).then(
      () => next(),
      (error: unknown) => {
