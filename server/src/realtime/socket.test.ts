...
function inbox(socket: ClientSocket, event: ServerEvent): TicketEventPayload[] {
  const received: TicketEventPayload[] = [];
  socket.on(event, (payload: TicketEventPayload) => received.push(payload));
  return received;
}
...
function rooms(client: ClientSocket): Set<string> {
  const serverSide = io.of(SOCKET_NAMESPACE).sockets.get(client.id ?? '');
  if (!serverSide) throw new Error('The server holds no socket for this client.');
...
async function eventually(check: () => boolean, ms = 750): Promise<boolean> {
...
async function raiseTicket(token: string, title: string): Promise<RaisedTicket> {
...
async function moveTicket(ticket: RaisedTicket, status: TicketStatus): Promise<void> {
...
async function comment(ticketId: string, body: string, visibility: CommentVisibility): Promise<void> {
