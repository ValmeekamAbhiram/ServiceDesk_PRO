...
import { QUEUE_ROOM, ServerEvent, ticketRoom, userRoom } from '@shared/socket';
import { moduleLogger } from '@/config/logger';

const log = moduleLogger('realtime');

let channel: Namespace | null = null;
 ...
 * the next file.
 */
export function setRealtimeChannel(next: Namespace | null): void {
  channel = next;
}

export function realtimeAttached(): boolean {
  return channel !== null;
}
 ...
 */
function emit(rooms: string | string[], event: ServerEvent, payload: unknown): void {
  if (!channel) return;
  try {
...
export function emitToUser(userId: string, event: ServerEvent, payload: unknown): void {
 ...
 */
export function emitTicketChange(ticketId: string, event: ServerEvent, payload: unknown): void {
 ...
 */
export function emitToStaff(event: ServerEvent, payload: unknown): void {
 ...
 */
export function emitToEveryone(event: ServerEvent, payload: unknown): void {
  if (!channel) return;
  try {
