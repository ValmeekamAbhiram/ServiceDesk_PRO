...
describe('GET /api/health', () => {
  ...
  it('reports the database as up while connected', async () => {
    const body: HealthDto = (await health()).body.data;
  ...
  it('reports simulated time, so a probe sees the clock the rest of the app sees', async () => {
    const before = new Date(((await health()).body.data as HealthDto).time).getTime();
    advanceMinutes(120);
    try {
      const after = new Date(((await health()).body.data as HealthDto).time).getTime();
  ...
  it('never exposes a secret', async () => {
    ...
    for (const key of ['JWT_SECRET', 'JWT_REFRESH_SECRET', 'SEED_PASSWORD', 'MONGO_URI']) {
      ...
      if (value && value.length >= 6) expect(serialised).not.toContain(value);
    }
