 ...
 */
import { AuditAction, AuditEntity, PRIORITIES } from '@shared/enums';
import { SLA_POLICY_KEY, SlaPolicy } from '@/models';
...
export async function getSlaPolicy() {
     ...
     */
    const policy = await SlaPolicy.findOneAndUpdate({ key: SLA_POLICY_KEY }, { $setOnInsert: { key: SLA_POLICY_KEY } }, { new: true, upsert: true, setDefaultsOnInsert: true });
    cached = policy;
    return policy;
}
...
export function toSlaPolicyDto(policy) {
 ...
 * update.
 */
export async function saveSlaPolicy(input, actor) {
    ...
    if (input.targets) {
        for (const [priority, budget] of Object.entries(input.targets)) {
            if (budget)
                existing.targets[priority] = { ...budget };
        }
