...
function patchPolicy(token, body) {
...
describe('validation', () => {
    ...
    it('refuses a timezone no runtime knows', async () => {
        expect((await patchPolicy(adminToken, { businessHours: { timezone: 'Mars/Olympus_Mons' } })).status).toBe(400);
    });
    ...
    it('refuses a budget of zero minutes', async () => {
        expect((await patchPolicy(adminToken, { targets: { [Priority.LOW]: { responseMinutes: 0 } } })).status).toBe(400);
    });
...
describe('the change is visible afterwards', () => {
    ...
    it('flattens a budget change into something a person can read', async () => {
        expect((await patchPolicy(adminToken, {
