...
async function makeUser(name, role, status = UserStatus.ACTIVE) {
    return User.create({
        name,
...
const sweepAt = (at) => sweepSlaStates(sweepActorAt(at));
/**
...
describe('what a sweep is allowed to touch', () => {
    ...
    it('never sweeps a resolved ticket', async () => {
        ...
        const resolved = await tickets.changeStatus(s.ticket.id, {
...
describe('who hears about it', () => {
    it('tells the assignee, and nobody else', async () => {
        ...
        await tickets.assign(s.ticket.id, { assigneeId: s.tech._id.toString(), version: s.ticket.version }, asAdmin);
        ...
        expect(rows.map((row) => String(row.recipientId))).not.toContain(s.employee._id.toString());
    });
    it('falls back to every active admin while nobody has picked it up', async () => {
        ...
        expect(recipients).toEqual([s.admins[0]._id.toString(), s.admins[1]._id.toString()].sort());
