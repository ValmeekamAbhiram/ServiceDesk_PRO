...
const businessHours = z
  ...
  .refine(
    (hours) =>
      hours.startMinute === undefined ||
      hours.endMinute === undefined ||
      hours.endMinute > hours.startMinute,
    { message: 'The working day must end after it starts.', path: ['endMinute'] }
  );
...
const targets = z.object(
  Object.fromEntries(PRIORITIES.map((priority) => [priority, target.optional()])) as Record<
    Priority,
    z.ZodOptional<typeof target>
  >
);
...
export type UpdateSlaPolicyInput = z.infer<typeof body>;
