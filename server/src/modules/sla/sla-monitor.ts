...
import { Ticket, User, type TicketDoc } from '@/models';
...
const OPTIMISTIC: SlaState[] = [SlaState.ON_TRACK, SlaState.AT_RISK];
...
export interface SlaSweepSummary {
  ...
  atRisk: number;
  breached: number;
  /** Tickets somebody edited mid-sweep. Left for the next tick; see `persist`. */
  skipped: number;
  durationMs: number;
}
 ...
 */
function sweepActor(): ActorContext {
  return {
 ...
 */
export async function sweepSlaStates(actor: ActorContext): Promise<SlaSweepSummary> {
  ...
  for (const ticket of tickets) {
    ...
    for (const event of crossed) {
      if (event.state === SlaState.BREACHED) summary.breached += 1;
      else summary.atRisk += 1;
    }

    if (crossed.length > 0 && !ticket.assigneeId) admins ??= await activeAdminIds();
    for (const event of crossed) {
...
function statesCrossed(
  ticket: TicketDoc,
  next: { response: SlaState; resolution: SlaState }
): { target: 'response' | 'resolution'; state: SlaState }[] {
  const crossed: { target: 'response' | 'resolution'; state: SlaState }[] = [];
...
async function persist(
  ...
  const result = await Ticket.updateOne(
    { _id: ticket._id, version: ticket.version },
    {
      $set: {
...
export function startSlaMonitor(): () => void {
  if (timer) return stopSlaMonitor;
  ...
  timer = setInterval(() => {
    ...
    void sweepSlaStates(sweepActor())
      ...
      .catch((error: unknown) => log.error({ err: error }, 'SLA sweep failed'))
      .finally(() => {
        running = false;
      });
  }, intervalMs);
