...
} from '@/modules/sla/sla-policy.schema';
...
slaRouter.get(
  '/',
  handler(async (_req, res) => ok(res, toSlaPolicyDto(await getSlaPolicy())))
);
...
slaRouter.patch(
  ...
  handler(async (req, res) =>
    ok(res, await saveSlaPolicy(bodyOf<UpdateSlaPolicyInput>(req), requireActor(req)))
  )
);
