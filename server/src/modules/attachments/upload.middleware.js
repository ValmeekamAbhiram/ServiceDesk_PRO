...
import { PayloadTooLargeError, UnsupportedMediaTypeError, UploadRejectedError, } from '@/utils/errors';
/**
...
function storedNameFor(extension) {
...
const upload = multer({
    ...
    fileFilter: (_req, file, done) => {
        ...
        if (!permitted.includes(extension)) {
            done(new UploadRejectedError(`A ${file.mimetype} file cannot have a "${extension}" extension.`, { mimeType: file.mimetype, extension, expected: permitted }));
            return;
        }
        done(null, true);
    },
 ...
 */
export function acceptFiles(field = 'files') {
    const inner = upload.array(field, MAX_FILES);
    return (req, res, next) => {
        inner(req, res, (error) => next(error ? translate(error) : undefined));
    };
}
/** The same, for routes that take exactly one file. */
export function acceptFile(field = 'file') {
 ...
 */
export function cleanupOnFailure() {
    return (req, res, next) => {
        res.on('finish', () => {
