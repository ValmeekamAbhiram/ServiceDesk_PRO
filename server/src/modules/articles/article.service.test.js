...
import { listArticlesSchema } from '@/modules/articles/article.schema';
beforeAll(startDb);
afterEach(clearDb);
afterAll(stopDb);
...
async function publish(id, reviewer) {
...
describe('writing an article', () => {
    it('always creates a draft, whatever the caller passes', async () => {
        ...
        const article = await articles.create(draft({ status: ArticleStatus.PUBLISHED, viewCount: 999 }), author);
    ...
    it('refuses a category that does not exist', async () => {
        const author = actorFor(await makeUser('Grace Hopper', Role.TECHNICIAN));
        await expect(articles.create(draft({ categoryId: '0123456789abcdef01234567' }), author)).rejects.toMatchObject({ statusCode: 400, code: 'VALIDATION_FAILED' });
    });
    it('links a real category and reports it as an option', async () => {
        const author = actorFor(await makeUser('Grace Hopper', Role.TECHNICIAN));
        const category = await Category.create({ name: 'Network', slug: 'network' });
        const article = await articles.create(draft({ categoryId: String(category._id) }), author);
    ...
    it('rejects a stale version on update', async () => {
        ...
        await expect(articles.update(article.id, { version: article.version, summary: 'Written from a stale form.' }, author)).rejects.toMatchObject({ statusCode: 409 });
    });
});
...
describe('the review boundary', () => {
    ...
    it('stops the author editing an article once it is published', async () => {
        ...
        await expect(articles.update(article.id, { version: live.version, body: 'Rewritten after approval.' }, author)).rejects.toMatchObject({ statusCode: 403 });
        /* The reviewer can, which is what makes the rule a boundary rather than a lock. */
        const edited = await articles.update(article.id, { version: live.version, body: 'Corrected by the reviewer.' }, admin);
...
describe('retracting', () => {
    it('keeps the original publication date and freezes the slug', async () => {
        ...
        const renamed = await articles.update(article.id, { version: retracted.version, title: 'A completely different heading' }, admin);
...
describe('search', () => {
    it('ranks a title match above a body match', async () => {
        const author = actorFor(await makeUser('Grace Hopper', Role.TECHNICIAN));
        const admin = actorFor(await makeUser('Ada Lovelace', Role.ADMIN));
        const inBody = await articles.create(draft({
        ...
        const inTitle = await articles.create(draft({
