...
import type { ArticleDto, ArticleSearchHitDto, Option, Paginated } from '@shared/types';
...
/** An article with its references either populated or left as ids; both map. */
type ArticleSource = Omit<KnowledgeArticleDoc, 'categoryId' | 'authorId'> & {
  categoryId: Populated<Pick<CategoryDoc, '_id' | 'name'>>;
  authorId: Populated<UserRefSource>;
};

function toCategoryOption(value: ArticleSource['categoryId']): Option | null {
  const category = populatedDoc(value, 'name');
  return category ? { id: String(category._id), label: category.name } : null;
}

export function toArticleDto(article: ArticleSource): ArticleDto {
  return {
    ...
    tags: article.tags,
    author: toUserRefDtoOrNull(populatedDoc(article.authorId, 'name') as never),
 ...
 */
function toSearchHit(article: KnowledgeArticleDoc & { score?: number }): ArticleSearchHitDto {
  return {
    id: String(article._id),
    title: article.title,
    slug: article.slug,
...
function isReviewer(actor: ActorContext): boolean {
  return can(actor, Permission.ARTICLE_PUBLISH);
}

function scopeFilter(actor: ActorContext): FilterQuery<KnowledgeArticleDoc> {
...
const SORT_FIELDS: Record<ListArticlesInput['sortBy'], string> = {
  updatedAt: 'updatedAt',
...
function buildListFilter(
  ...
  actor: ActorContext
): FilterQuery<KnowledgeArticleDoc> {
  const clauses: FilterQuery<KnowledgeArticleDoc>[] = [scopeFilter(actor)];
  if (query.status) clauses.push({ status: query.status });
  if (query.categoryId) clauses.push({ categoryId: toObjectId(query.categoryId) });
  if (query.tag) clauses.push({ tags: query.tag });
  if (query.q) clauses.push({ $text: { $search: query.q } });
  return { $and: clauses };
}
...
export async function list(
  ...
  const cursor = KnowledgeArticle.find(filter)
    .populate(ARTICLE_POPULATE as never)
    .sort(sort as never)
    .skip(skip)
    .limit(limit);
  ...
  return toPaginated(
    rows.map((row) => toArticleDto(row as unknown as ArticleSource)),
    page,
    limit,
    total
  );
 ...
 */
export async function search(query: SearchArticlesInput): Promise<ArticleSearchHitDto[]> {
  const rows = await KnowledgeArticle.find({
    status: ArticleStatus.PUBLISHED,
    $text: { $search: query.q },
  })
    .select({ title: 1, slug: 1, summary: 1, tags: 1, score: { $meta: 'textScore' } })
    .sort({ score: { $meta: 'textScore' }, _id: -1 } as never)
    .limit(query.limit)
    .lean();

  return rows.map((row) => toSearchHit(row as unknown as KnowledgeArticleDoc & { score: number }));
}
...
export async function getById(id: string, actor: ActorContext): Promise<ArticleDto> {
  ...
  return toArticleDto(article as unknown as ArticleSource);
}
 ...
 */
async function uniqueSlug(title: string, excludeId?: KnowledgeArticleDoc['_id']): Promise<string> {
...
export async function create(input: ArticleInputBody, actor: ActorContext): Promise<ArticleDto> {
  ...
  await article.populate(ARTICLE_POPULATE as never);
  log.info(
    { requestId: actor.requestId, articleId: String(article._id), slug: article.slug },
    'article created'
  );
  await record(
    {
 ...
 */
async function loadForEdit(id: string, actor: ActorContext) {
  const article = await loadScoped(id, actor);
 ...
 */
const AUDITED_ARTICLE_FIELDS = ['title', 'slug', 'summary', 'body', 'categoryId', 'tags'] as const;

function auditSnapshot(article: KnowledgeArticleDoc): Record<string, unknown> {
  return {
    title: article.title,
    slug: article.slug,
...
export async function setPublished(
  ...
  if (article.status === target) return toArticleDto(article as unknown as ArticleSource);

  if (publish) assertVersion(expectedVersion, article.version, 'Article');
  ...
  if (publish) article.publishedAt = article.publishedAt ?? actor.clock.now();

  await article.save();
  log.info(
    { requestId: actor.requestId, articleId: id, status: target },
    publish ? 'article published' : 'article retracted'
  );
