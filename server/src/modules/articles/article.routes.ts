...
articleRouter.get(
  ...
  handler(async (req, res) =>
    paginated(res, await articleService.list(queryOf<ListArticlesInput>(req), requireActor(req)))
  )
);
...
articleRouter.get(
  '/search',
  requirePermission(Permission.ARTICLE_READ),
  validate(searchArticlesSchema),
  handler(async (req, res) =>
    ok(res, await articleService.search(queryOf<SearchArticlesInput>(req)))
  )
);
...
articleRouter.get(
  ...
  handler(async (req, res) =>
    ok(res, await articleService.getById(paramsOf<{ id: string }>(req).id, requireActor(req)))
  )
);

articleRouter.post(
  '/',
  requirePermission(Permission.ARTICLE_WRITE),
  validate(createArticleSchema),
  handler(async (req, res) => {
    const article = await articleService.create(bodyOf<ArticleInputBody>(req), requireActor(req));
...
articleRouter.post(
  '/:id/publish',
  requirePermission(Permission.ARTICLE_PUBLISH),
  validate(publishArticleSchema),
  handler(async (req, res) =>
    ok(
      res,
      await articleService.setPublished(
        paramsOf<{ id: string }>(req).id,
        true,
        requireActor(req),
        bodyOf<PublishArticleInput>(req).version
      )
    )
  )
);
...
articleRouter.post(
  ...
  validate(articleIdSchema),
  handler(async (req, res) =>
    ok(res, await articleService.setPublished(paramsOf<{ id: string }>(req).id, false, requireActor(req)))
  )
);
