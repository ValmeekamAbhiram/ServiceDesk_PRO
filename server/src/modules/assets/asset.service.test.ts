...
import { listAssetsSchema, type ListAssetsInput } from '@/modules/assets/asset.schema';
...
function listQuery(raw: Record<string, unknown> = {}): ListAssetsInput {
...
describe('creating an asset', () => {
  ...
  it('refuses a duplicate serial number but allows many without one', async () => {
    const admin = actorFor(await makeUser('Ada Lovelace', Role.ADMIN));
    await assets.create(
      { name: 'ThinkPad X1', type: AssetType.LAPTOP, serialNumber: 'PF-0AA111' },
      admin
    );

    await expect(
      assets.create({ name: 'Another X1', type: AssetType.LAPTOP, serialNumber: 'PF-0AA111' }, admin)
...
describe('an employee sees only what they hold', () => {
  it('lists their own hardware and nothing else', async () => {
    ...
    const hers = await assets.create(
      { name: 'ThinkPad X1', type: AssetType.LAPTOP, assignedToId: grace._id.toString() },
      admin
    );
    await assets.create(
      { name: 'MacBook Air', type: AssetType.LAPTOP, assignedToId: alan._id.toString() },
      admin
    );
  ...
  it('cannot be widened with a forged assignedToId', async () => {
    ...
    const forged = await assets.list(
      listQuery({ assignedToId: alan._id.toString() }),
      actorFor(grace)
    );
...
describe('retired hardware cannot also be issued', () => {
  it('releases the holder when the asset is retired', async () => {
    ...
    const retired = await assets.update(
      asset.id,
      { status: AssetStatus.RETIRED, version: asset.version },
      admin
    );
  ...
  it('refuses to issue a retired asset, from either direction', async () => {
    ...
    await expect(
      assets.update(
        asset.id,
        { assignedToId: grace._id.toString(), version: retired.version },
        admin
      )
    ).rejects.toMatchObject({ statusCode: 409 });
  });
  ...
  it('will not issue an asset to a deactivated or unknown person', async () => {
    ...
    await expect(
      assets.create(
        { name: 'ThinkPad X1', type: AssetType.LAPTOP, assignedToId: gone._id.toString() },
        admin
      )
    ).rejects.toThrow(/cannot hold an asset/i);
  });
});
...
describe('concurrent edits and the warranty clock', () => {
  it('rejects a stale version', async () => {
    ...
    await expect(
      assets.update(asset.id, { location: 'Floor 3', version: asset.version }, admin)
    ).rejects.toMatchObject({ statusCode: 409 });
  });
  ...
  it('the radar finds expiring and expired kit, and skips assets with no warranty', async () => {
    ...
    const radar = await assets.list(
      listQuery({ warrantyWithinDays: 30, sortBy: 'warrantyExpiryDate' }),
      admin
    );
