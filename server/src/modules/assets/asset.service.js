...
import { Asset, User, nextSequence, toObjectId } from '@/models';
...
export function formatAssetTag(sequence) {
 ...
 */
function warrantyDaysRemaining(expiry, now) {
    if (!expiry)
        return null;
    return Math.ceil((expiry.getTime() - now.getTime()) / MS_PER_DAY);
}
export function toAssetDto(asset, now) {
    return {
        id: String(asset._id),
        tag: asset.tag,
        name: asset.name,
        ...
        assignedTo: assignedToRef(asset.assignedToId),
 ...
 */
function assignedToRef(value) {
    return toUserRefDtoOrNull(populatedDoc(value, 'name'));
}
/* ──────────────────────────────── reading ───────────────────────────────── */
/**
...
const ASSET_POPULATE = { path: 'assignedToId', select: 'name email role status' };
const SORT_FIELDS = {
    name: 'name',
    tag: 'tag',
    createdAt: 'createdAt',
...
function buildListFilter(query, actor) {
    ...
    if (query.type)
        clauses.push({ type: { $in: query.type } });
    if (query.status)
        clauses.push({ status: { $in: query.status } });
    if (query.assignedToId)
        clauses.push({ assignedToId: toObjectId(query.assignedToId) });
...
export async function list(query, actor) {
    ...
    if (query.q)
        Object.assign(filter, { $text: { $search: query.q } });
    ...
    return toPaginated(rows.map((row) => toAssetDto(row, now)), page, limit, total);
}
/**
...
export async function getById(id, actor) {
    ...
    return toAssetDto(asset, actor.clock.now());
}
/* ──────────────────────────────── writing ───────────────────────────────── */
/**
 ...
 */
async function assertSerialFree(serial, excludeId) {
    if (!serial)
        return;
 ...
 */
async function resolveHolder(userId) {
...
export async function update(id, input, actor) {
    ...
    if (input.assignedToId !== undefined) {
        asset.assignedToId = (input.assignedToId ? await resolveHolder(input.assignedToId) : null);
    }
    ...
    if (input.warrantyExpiryDate !== undefined)
        asset.warrantyExpiryDate = input.warrantyExpiryDate;
    ...
    await asset.save();
    log.info({ requestId: actor.requestId, assetId: id, status: asset.status }, 'asset updated');
    await record({
