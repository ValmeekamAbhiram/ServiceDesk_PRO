...
assetRouter.get(
  ...
  handler(async (req, res) =>
    paginated(res, await assetService.list(queryOf<ListAssetsInput>(req), requireActor(req)))
  )
);
...
assetRouter.get(
  ...
  handler(async (req, res) =>
    ok(res, await assetService.getById(paramsOf<{ id: string }>(req).id, requireActor(req)))
  )
);
...
assetRouter.post(
  ...
  handler(async (req, res) => {
    const asset = await assetService.create(bodyOf<AssetInputBody>(req), requireActor(req));
