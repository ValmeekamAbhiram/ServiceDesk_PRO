...
function contextOf(req: Request): authService.AuthContext {
  return {
    clock: getClock(),
    ip: req.ip ?? null,
...
export const register = handler(async (req, res) => {
  const result = await authService.register(bodyOf<RegisterInput>(req), contextOf(req));
  return created(res, result);
});

export const login = handler(async (req, res) => {
  const result = await authService.login(bodyOf<LoginInput>(req), contextOf(req));
  return ok(res, result);
});

export const refresh = handler(async (req, res) => {
  const result = await authService.refresh(bodyOf<RefreshInput>(req), contextOf(req));
  return ok(res, result);
});
...
export const changePassword = handler(async (req, res) => {
  const result = await authService.changePassword(
    requireActor(req),
    bodyOf<ChangePasswordInput>(req)
  );
  return ok(res, result);
});
