...
import type { AuthResponse, AuthTokens, CurrentUserDto } from '@shared/types';
...
import { toCurrentUserDto, type UserSource } from '@/modules/users/user.mapper';
...
/* ──────────────────────────────── context ─────────────────────────────────
 * Register, login and refresh happen *before* there is an authenticated user, so
 * they cannot take an `ActorContext`. They take this narrower shape instead —
 * which `ActorContext` structurally satisfies, so an authenticated caller can
 * pass its actor straight through.
 * ------------------------------------------------------------------------- */
 ...
 */
let timingDecoy: string | null = null;

async function equaliseFailureTiming(candidate: string): Promise<void> {
...
function deviceLabelFrom(userAgent: string | null): string | null {
...
/* ────────────────────────────── session issuing ───────────────────────────
 * One place mints credentials, so the refresh token exists in plaintext in
 * exactly one expression and only its SHA-256 is ever persisted.
 * ------------------------------------------------------------------------- */
...
async function revokeFamily(familyId: string, at: Date, reason: string): Promise<number> {
...
function authResponse(user: UserSource, tokens: AuthTokens): AuthResponse {
 ...
 */
function actorForAccount(user: HydratedDocument<UserDoc>, ctx: AuthContext): ActorContext {
  return {
    user: {
...
export async function register(input: RegisterInput, ctx: AuthContext): Promise<AuthResponse> {
  ...
  log.info(
    { requestId: ctx.requestId, userId: user._id.toString(), role: user.role, isFirstAccount },
    'Account registered'
  );
...
export async function login(input: LoginInput, ctx: AuthContext): Promise<AuthResponse> {
  ...
  if (user.status !== UserStatus.ACTIVE) {
    throw new ForbiddenError(
      'This account is not active. Please contact your IT administrator.'
    );
  }
...
export async function logout(actor: ActorContext): Promise<void> {
  ...
  await Session.updateOne(
    ...
    { $set: { revokedAt: actor.clock.now(), revokedReason: 'signed out' } }
  );
 ...
 * working is not.
 */
export async function refresh(input: RefreshInput, ctx: AuthContext): Promise<AuthResponse> {
  const session = await Session.findOne({
    refreshTokenHash: hashRefreshToken(input.refreshToken),
  }).select('+refreshTokenHash');

  // Unknown, or already swept by the TTL index on `expiresAt`.
  if (!session) throw new TokenInvalidError('Please sign in again.');
...
export async function changePassword(
  ...
  log.info(
    { requestId: actor.requestId, userId: user._id.toString(), sessionsRevoked: revoked.modifiedCount ?? 0 },
 ...
 */
export async function currentUser(actor: ActorContext): Promise<CurrentUserDto> {
