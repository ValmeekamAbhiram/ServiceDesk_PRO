...
import { changePassword, currentUser, login, logout, refresh, register, } from '@/modules/auth/auth.service';
...
function actorFor(user, sessionId) {
    return {
        user: {
            ...user,
...
describe('register', () => {
    ...
    it('ignores a role supplied by the client', async () => {
        ...
        const result = await register({ ...CREDENTIALS, role: Role.ADMIN }, ctx());
        expect(result.user.role).toBe(Role.EMPLOYEE);
    });
...
describe('login', () => {
    ...
    it('refuses a deactivated account, and only after the password verifies', async () => {
        ...
        await expect(login({ email: CREDENTIALS.email, password: CREDENTIALS.password }, ctx())).rejects.toMatchObject({ statusCode: 403 });
        ...
        await expect(login({ email: CREDENTIALS.email, password: 'Wr0ngPass!' }, ctx())).rejects.toMatchObject({ statusCode: 401 });
    });
...
describe('refresh', () => {
    ...
    it('rejects an unknown token without creating anything', async () => {
        await register(CREDENTIALS, ctx());
        await expect(refresh({ refreshToken: 'not-a-real-refresh-token' }, ctx())).rejects.toMatchObject({ statusCode: 401 });
...
describe('changePassword', () => {
    it('rejects a wrong current password with a field error, not a 401', async () => {
        ...
        await expect(changePassword(actor, { currentPassword: 'Wr0ngPass!', newPassword: 'BrandNew1' })).rejects.toMatchObject({ statusCode: 400, fields: [{ path: 'currentPassword' }] });
    });
    it('replaces the password, revokes every session and hands back working tokens', async () => {
        ...
        await expect(login({ email: CREDENTIALS.email, password: CREDENTIALS.password }, ctx())).rejects.toMatchObject({ statusCode: 401 });
        await expect(login({ email: CREDENTIALS.email, password: 'BrandNew1' }, ctx())).resolves.toBeTruthy();
