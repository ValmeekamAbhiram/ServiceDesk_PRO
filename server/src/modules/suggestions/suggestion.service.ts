 ...
 * such excuse.
 */
function textQueryFrom(title: string, description?: string | null): string {
 ...
 */
async function relatedArticles(query: string): Promise<TicketSuggestionDto['relatedArticles']> {
...
export async function suggestTicket(
  actor: ActorContext,
  input: TicketSuggestionRequest
): Promise<TicketSuggestionDto> {
