 ...
 */
function mentionsUnnegated(haystack, phrase) {
    const needle = normalize(phrase);
    if (needle === ' ' || needle === '  ')
        return false;
    let from = 0;
    while (from <= haystack.length) {
        ...
        if (!NEGATORS.has(words[words.length - 1] ?? ''))
            return true;
        from = at + 1;
    }
    return false;
...
function firstSignal(text, signals) {
    for (const signal of signals)
        if (mentionsUnnegated(text, signal))
            return signal;
    return null;
}
const byRank = (a, b) => PRIORITY_RANK[a] >= PRIORITY_RANK[b] ? a : b;
 ...
 */
function scoreCategories(title, body, candidates) {
 ...
 */
export function classify(input, candidates) {
    ...
    let priority = base;
    if (urgent)
        priority = byRank(base, Priority.URGENT);
    else if (high)
        priority = byRank(base, Priority.HIGH);
    else if (calm)
        priority = SOFTER[base] ?? base;
    const notes = [];
    if (urgent)
        notes.push(`"${urgent}" reads as an emergency.`);
    else if (high)
        notes.push(`"${high}" reads as time-critical.`);
    else if (calm && priority !== base)
        notes.push(`"${calm}" reads as low priority.`);
    if (!winner) {
        return {
            categoryId: null,
            categoryName: null,
    ...
    const confidence = round2(clamp(MIN_CONFIDENCE + 0.4 * share + 0.2 * depth, MIN_CONFIDENCE, MAX_CONFIDENCE));
