...
import type { CategoryCandidate, Classification } from '@/modules/suggestions/suggestion.classifier';
...
function describeCandidates(candidates: readonly CategoryCandidate[]): string {
  return candidates
    .map((candidate) => {
...
export async function suggestWithLlm(
  ...
  if (env.aiProvider !== 'anthropic' || candidates.length === 0) return null;
  ...
  try {
    ...
    log.warn(
      { timedOut },
      timedOut ? 'Suggestion provider timed out.' : 'Suggestion provider is unreachable.'
    );
    return null;
  }
}
