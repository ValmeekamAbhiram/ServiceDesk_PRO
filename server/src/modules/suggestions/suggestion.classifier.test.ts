...
import { classify, type CategoryCandidate } from '@/modules/suggestions/suggestion.classifier';

const NETWORK: CategoryCandidate = {
  id: 'cat-network',
  name: 'Network',
...
const HARDWARE: CategoryCandidate = {
  id: 'cat-hardware',
  name: 'Hardware',
...
const ACCOUNTS: CategoryCandidate = {
  id: 'cat-accounts',
  name: 'Accounts',
...
const SECURITY: CategoryCandidate = {
  id: 'cat-security',
  name: 'Security',
...
describe('picking a category', () => {
  ...
  it('matches the category name itself, with no keywords configured', () => {
    const bare: CategoryCandidate = { ...HARDWARE, keywords: [] };
    expect(classify({ title: 'Hardware request for a new desk' }, [bare]).categoryId).toBe(
      'cat-hardware'
    );
  });

  it('matches a multi-word keyword as a phrase', () => {
    expect(classify({ title: 'Laptop shows a blue screen on boot' }, ALL).categoryId).toBe(
      'cat-hardware'
    );
  });
  ...
  it('does not match a keyword buried inside a longer word', () => {
    ...
    const ports: CategoryCandidate = { ...NETWORK, keywords: ['port'] };
  ...
  it('prefers the title over the description, keyword for keyword', () => {
    const result = classify(
      { title: 'Laptop is broken', description: 'I also cannot reach the internet' },
      ALL
    );
  ...
  it('still lets weight of evidence in the description compete', () => {
     ...
     */
    const result = classify(
      { title: 'Laptop is broken', description: 'I also cannot connect to the wifi or vpn' },
      ALL
    );
  ...
  it('breaks a tie on the order the admin arranged the categories in', () => {
    ...
    expect(classify({ title: 'password reset' }, [ACCOUNTS, SECURITY]).categoryId).toBe(
      'cat-accounts'
    );
    expect(classify({ title: 'password reset' }, [SECURITY, ACCOUNTS]).categoryId).toBe(
      'cat-security'
    );
  });

  it('ignores an empty keyword instead of matching everything', () => {
    const sloppy: CategoryCandidate = { ...HARDWARE, name: 'Hardware', keywords: ['', '   '] };
...
describe('suggesting a priority', () => {
  ...
  it('raises a low-default category when the wording is an emergency', () => {
    const result = classify(
      { title: 'Cannot login', description: 'nobody in the entire office can sign in' },
      [ACCOUNTS]
    );
...
describe('confidence', () => {
  it('never claims certainty, however many keywords match', () => {
    const result = classify(
      { title: 'vpn wifi internet ethernet cannot connect', description: 'vpn wifi internet' },
      ALL
    );
    expect(result.categoryId).toBe('cat-network');
...
describe('negated urgency', () => {
  ...
  it('does not read "nothing is urgent here" as urgent', () => {
    expect(classify({ title: 'keyboard swap, nothing urgent' }, [HARDWARE]).priority).toBe(
      Priority.MEDIUM
    );
  });
  ...
  it('does not read "no outage" as an emergency', () => {
    const result = classify(
      { title: 'wifi question', description: 'there is no outage, just curious' },
      [NETWORK]
    );
    expect(result.priority).toBe(Priority.HIGH);
  ...
  it('still fires on a later un-negated occurrence', () => {
    ...
    const result = classify(
      { title: 'Access request', description: 'not urgent yesterday but it is urgent now' },
      [ACCOUNTS]
    );
    expect(result.priority).toBe(Priority.HIGH);
  });
