...
function actorFor(user: HydratedDocument<UserDoc>): ActorContext {
  return {
    user: {
...
let seq = 0;
async function person(role: Role, name: string): Promise<ActorContext> {
  seq += 1;
...
async function raise(requester: ActorContext, categoryId: string) {
  return tickets.create({ title: 'Wi-Fi keeps dropping', description: 'Every few minutes.', categoryId }, requester);
}

async function bell(actor: ActorContext) {
...
describe('who hears about a ticket', () => {
  ...
  it('tells both the requester and the assignee when a third person moves it', async () => {
    ...
    const assigned = await tickets.assign(
      ticket.id,
      { assigneeId: tech.user.id, version: ticket.version },
      admin
    );
    await tickets.changeStatus(
      ticket.id,
      { status: TicketStatus.IN_PROGRESS, version: assigned.version },
      admin
    );

    const statusOnly = (menu: Awaited<ReturnType<typeof bell>>) =>
      menu.items.filter((row) => row.type === NotificationType.TICKET_STATUS_CHANGED);
...
describe('comments, and the internal note rule', () => {
  it('tells the requester about a public reply', async () => {
    ...
    await tickets.addComment(
      ticket.id,
      { body: 'Have you tried the other network?', visibility: CommentVisibility.PUBLIC },
      tech
    );
  ...
  it('never tells the requester about an internal note, by text or by count', async () => {
    ...
    await tickets.addComment(
      assigned.id,
      { body: 'Requester has broken this twice; check the switch port.', visibility: CommentVisibility.INTERNAL },
      admin
    );
  ...
  it('does not notify the requester about their own comment', async () => {
    ...
    await tickets.addComment(
      ticket.id,
      { body: 'Any update?', visibility: CommentVisibility.PUBLIC },
      employee
    );
...
describe('the bell menu', () => {
  ...
  async function twoFor(employee: ActorContext, tech: ActorContext): Promise<void> {
    const ticket = await raise(employee, await category());
    const moved = await tickets.changeStatus(
      ticket.id,
      { status: TicketStatus.IN_PROGRESS, version: ticket.version },
      tech
    );
    await tickets.addComment(
      moved.id,
      { body: 'Looking into it now.', visibility: CommentVisibility.PUBLIC },
      tech
    );
  }
...
describe('whose notifications these are', () => {
  ...
  async function oneForEmployee(employee: ActorContext, tech: ActorContext): Promise<string> {
    const ticket = await raise(employee, await category());
    await tickets.changeStatus(
      ticket.id,
      { status: TicketStatus.IN_PROGRESS, version: ticket.version },
