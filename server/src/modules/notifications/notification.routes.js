...
import { listNotificationsSchema, markAllReadSchema, notificationIdSchema, } from '@/modules/notifications/notification.schema';
import { handler } from '@/utils/handler';
import { paramsOf, queryOf } from '@/utils/input';
import { ok } from '@/utils/respond';
export const notificationRouter = Router();
notificationRouter.use(authenticate());
notificationRouter.get('/', validate(listNotificationsSchema), handler(async (req, res) => ok(res, await notificationService.list(queryOf(req), requireActor(req)))));
/*
 ...
 */
notificationRouter.post('/read-all', validate(markAllReadSchema), handler(async (req, res) => ok(res, await notificationService.markAllRead(queryOf(req), requireActor(req)))));
notificationRouter.post('/:id/read', validate(notificationIdSchema), handler(async (req, res) => ok(res, await notificationService.markRead(paramsOf(req).id, requireActor(req)))));
