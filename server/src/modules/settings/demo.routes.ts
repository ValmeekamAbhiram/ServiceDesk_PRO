...
import {
  ...
  jumpTo,
  reset,
} from '@/modules/settings/demo.service';
import {
  advanceClockSchema,
  setClockSchema,
...
demoRouter.get(
  '/clock',
  handler(async (_req, res) => ok(res, currentClock()))
);

demoRouter.post(
  '/clock/advance',
  validate(advanceClockSchema),
  handler(async (req, res) =>
    ok(res, await advance(bodyOf<AdvanceClockInput>(req).minutes, requireActor(req)))
  )
);

demoRouter.post(
  '/clock/set',
  validate(setClockSchema),
  handler(async (req, res) =>
    ok(res, await jumpTo(bodyOf<SetClockInput>(req).at, requireActor(req)))
  )
);

demoRouter.post(
  '/clock/reset',
  handler(async (req, res) => ok(res, await reset(requireActor(req))))
);
