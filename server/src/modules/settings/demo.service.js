...
import { advanceMinutes, clockState, offsetLabel, resetClock, setAbsolute, } from '@/config/clock';
 ...
 */
async function announce(previousOffsetMs, summary, actor) {
    const state = clockState();
    log.warn({ requestId: actor.requestId, byUserId: actor.user.id, offsetMs: state.offsetMs }, 'Demo clock changed');
    await record({
...
export async function advance(minutes, actor) {
    await assertDemoAvailable();
    const previous = clockState().offsetMs;
    advanceMinutes(minutes);
    const direction = minutes < 0 ? 'back' : 'forward';
    return announce(previous, `Moved the demo clock ${direction} by ${offsetLabel(Math.abs(minutes) * 60_000)}`, actor);
}
export async function jumpTo(instant, actor) {
    await assertDemoAvailable();
    const previous = clockState().offsetMs;
    setAbsolute(instant);
