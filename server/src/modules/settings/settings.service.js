...
import { SETTINGS_KEY, SystemSettings, toObjectId } from '@/models';
...
export async function getSystemSettings() {
    ...
    const settings = await SystemSettings.findOneAndUpdate({ key: SETTINGS_KEY }, { $setOnInsert: { key: SETTINGS_KEY } }, { new: true, upsert: true, setDefaultsOnInsert: true });
    cached = settings;
    return settings;
}
...
export async function demoModeActive() {
    if (!env.demoMode)
        return false;
 ...
 */
export async function saveSettings(input, actor) {
    ...
    if (input.demoModeRequested !== undefined)
        settings.demoModeRequested = input.demoModeRequested;
    ...
    const closingTheDoor = input.demoModeRequested === false && env.demoMode && isTimeShifted();
    if (closingTheDoor)
        resetClock();
    await settings.save();
