...
const advanceBody = z.object({
  minutes: z
    ...
    .refine(
      (value) => Math.abs(value) <= MAX_JUMP_MINUTES,
      'Jumps are limited to fourteen days at a time.'
    ),
});

export const advanceClockSchema = { body: advanceBody };
export type AdvanceClockInput = z.infer<typeof advanceBody>;
...
export type SetClockInput = z.infer<typeof setClockBody>;
