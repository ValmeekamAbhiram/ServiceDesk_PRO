 ...
 * calls this.
 */
async function reauthenticateAdmin(): Promise<void> {
  const response = await request(app)
...
function patchSettings(token: string, body: Record<string, unknown>) {
  return request(app).patch('/api/settings').set(authed(token)).send(body);
}

function advanceClock(token: string, minutes: number) {
