...
describe('the trail is append-only', () => {
    ...
    it('refuses an update made directly against the model', async () => {
        ...
        await expect(AuditLog.updateOne({ _id: row._id }, { $set: { summary: 'Something else' } })).rejects.toThrow();
...
describe('what a change looks like in the trail', () => {
    it('names both ends of a role change and files it under ROLE_CHANGED', async () => {
        ...
        const entry = response.body.data.items.find((row) => row.action === AuditAction.ROLE_CHANGED);
...
describe('filters', () => {
    it('narrows by action, and says so in the total', async () => {
        ...
        expect(response.body.data.meta.total).toBe(await AuditLog.countDocuments({ action: AuditAction.LOGIN }));
    });
    ...
    it('returns the newest entry first', async () => {
        const items = (await trail()).body.data.items;
