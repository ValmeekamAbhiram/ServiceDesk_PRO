...
import { Role, type AuditAction, type AuditEntity } from '@shared/enums';
import type { AuditLogDto, Paginated } from '@shared/types';
import { moduleLogger } from '@/config/logger';
import { isSystemActor, type ActorContext } from '@/core/actor';
import { AuditLog, type AuditChangeDoc, type AuditLogDoc } from '@/models';
import { toObjectId } from '@/models/helpers';
import { resolvePaging, toPaginated } from '@/utils/respond';
import type { ListAuditInput } from '@/modules/audit/audit.schema';
...
export interface AuditEntryInput {
  ...
  /** `TKT-000123`, a category name — whatever a person would recognise. */
  entityLabel?: string | null;
  /** One line, past tense: "Assigned to Tara Tech". */
  summary: string;
 ...
 */
export async function record(entry: AuditEntryInput, actor: ActorContext): Promise<void> {
  try {
    ...
    log.error({ err: error, action: entry.action, entityId: entry.entityId },
      'Audit entry could not be written; the operation it describes still happened.');
  }
}
 ...
 * silently drop them.
 */
export async function list(query: ListAuditInput): Promise<Paginated<AuditLogDto>> {
  ...
  return toPaginated(rows.map((row) => toAuditLogDto(row as AuditLogDoc)), page, limit, total);
}
 ...
 */
export function toAuditLogDto(row: AuditLogDoc): AuditLogDto {
  const changes =
  ...
  return {
    ...
    actor: row.actorId
      ? {
          ...
          role: (row.actorRole ?? Role.EMPLOYEE) as Role,
        }
      : null,
