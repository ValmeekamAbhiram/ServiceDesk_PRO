...
import { createCategorySchema, listCategoriesSchema, updateCategorySchema, } from '@/modules/categories/category.schema';
...
categoryRouter.get('/', validate(listCategoriesSchema), handler(async (req, res) => {
    const actor = requireActor(req);
    const asked = queryOf(req).includeInactive === true;
    return ok(res, await categoryService.list({
        includeInactive: asked && can(actor, Permission.SETTINGS_MANAGE),
    }));
}));
categoryRouter.post('/', requirePermission(Permission.SETTINGS_MANAGE), validate(createCategorySchema), handler(async (req, res) => created(res, await categoryService.create(bodyOf(req), requireActor(req)))));
categoryRouter.patch('/:id', requirePermission(Permission.SETTINGS_MANAGE), validate(updateCategorySchema), handler(async (req, res) => ok(res, await categoryService.update(paramsOf(req).id, bodyOf(req), requireActor(req)))));
