 ...
 */
import { OPEN_TICKET_STATUSES, Permission, PRIORITIES, SlaState, STAFF_ROLES, TICKET_STATUSES, UserStatus, } from '@shared/enums';
import { PRIORITY_META, TICKET_STATUS_META } from '@shared/labels';
import { DAY_MS } from '@shared/utils';
import { can } from '@/core/actor';
import { businessDateKey, businessMinutesBetween, evaluateTicketSla, recentBusinessDays, } from '@/core/sla';
import { Category, Ticket, User } from '@/models';
import { TICKET_POPULATE, ticketMapContext, ticketScopeFilter, } from '@/modules/tickets/ticket.service';
import { toTicketListItemDto, } from '@/modules/tickets/ticket.mapper';
import { toUserRefDto } from '@/modules/users/user.mapper';
/* ──────────────────────────────── shape knobs ───────────────────────────────── */
...
function countsFrom(counts, keys, labels) {
    return keys.map((key) => ({
        key,
 ...
 */
async function categoryCounts(rows) {
    if (rows.length === 0)
        return [];
 ...
 */
async function recentRows(scope, from) {
 ...
 */
function volumeSeries(rows, keys, hours) {
    ...
    for (const row of rows) {
        ...
        if (row.resolvedAt)
            bump(resolved, businessDateKey(row.resolvedAt, hours));
    }
...
const emptyWindow = () => ({ resolved: 0, compliant: 0, businessMinutes: 0 });
/**
 ...
 */
function resolutionMinutes(row, hours) {
    if (!row.resolvedAt)
        return 0;
 ...
 */
function accumulate(into, row, now, policy) {
    into.resolved += 1;
    if (!evaluateTicketSla(row.sla, now, policy).breached)
        into.compliant += 1;
...
const compliancePercent = (window) => window.resolved === 0 ? 0 : round1((window.compliant / window.resolved) * 100);
const avgHours = (window) => window.resolved === 0 ? 0 : round1(window.businessMinutes / window.resolved / 60);
function buildKpis(open, current, previous) {
 ...
 */
async function openLoad(scope, now) {
    const overdue = {
        $and: [
 ...
 */
async function technicianLoad(scope, rows, now, policy) {
    ...
    for (const row of rows) {
        if (!row.assigneeId || !row.resolvedAt || row.resolvedAt < since)
            continue;
    ...
    return staff
        ...
        return {
            technician: toUserRefDto(person),
            open: queue?.open ?? 0,
 ...
 */
async function atRiskTickets(scope, ctx) {
    const rows = await Ticket.find({
        $and: [
            scope,
    ...
    for (const row of rows) {
        ...
        if (state !== SlaState.AT_RISK && state !== SlaState.BREACHED)
            continue;
        urgent.push(toTicketListItemDto(row, ctx));
        if (urgent.length === AT_RISK_LIMIT)
            break;
    }
    return urgent;
}
...
export async function getDashboard(actor) {
    ...
    const open = OPEN_TICKET_STATUSES.reduce((total, status) => total + (statusCounts.get(status) ?? 0), 0);
    const current = emptyWindow();
    const previous = emptyWindow();
    for (const row of rows) {
        if (!row.resolvedAt)
            continue;
        if (row.resolvedAt >= currentFrom)
            accumulate(current, row, ctx.now, ctx.policy);
        else if (row.resolvedAt >= previousFrom)
            accumulate(previous, row, ctx.now, ctx.policy);
    }
    return {
