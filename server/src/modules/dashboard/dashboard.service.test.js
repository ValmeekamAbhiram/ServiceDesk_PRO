...
import { PRIORITIES, Priority, Role, TICKET_STATUSES, TicketStatus, UserStatus, } from '@shared/enums';
...
async function raise(requester, categoryId, options = {}) {
    ...
    if (options.createdAt)
        await patchRaw(ticket.id, { createdAt: new Date(options.createdAt) });
    return ticket.id;
}
/**
 ...
 */
async function patchRaw(id, fields) {
...
async function resolveTicket(id, staff, options = {}) {
    ...
    if (options.resolvedAt)
        await patchRaw(id, { resolvedAt: new Date(options.resolvedAt) });
}
/** Push a resolution deadline into the past, so the engine calls the target breached. */
async function overdue(id, dueAt = '2026-03-09T06:00:00.000Z') {
...
describe('volume series', () => {
    ...
    it('buckets a ticket into the business day, not the UTC one', async () => {
        ...
        const { volume } = await getDashboard(actorFor(employee));
        const on = (date) => volume.find((point) => point.date === date)?.created ?? -1;
    ...
    it('counts a resolution on the day it was resolved, not the day it arrived', async () => {
        ...
        const point = (date) => volume.find((entry) => entry.date === date);
...
describe('KPIs', () => {
    it('compares resolutions against the previous seven days', async () => {
        ...
        for (const id of thisWeek)
            await resolveTicket(id, tech, { resolvedAt: '2026-03-08T06:00:00.000Z' });
