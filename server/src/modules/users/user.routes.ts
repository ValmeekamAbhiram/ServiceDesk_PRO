...
userRouter.get(
  ...
  handler(async (req, res) => paginated(res, await userService.list(queryOf<ListUsersInput>(req))))
);

userRouter.get(
  '/assignees',
  requirePermission(Permission.USER_READ),
  handler(async (_req, res) => ok(res, await userService.listAssignees()))
);
...
userRouter.patch(
  ...
  handler(async (req, res) => {
    ...
    return ok(res, await userService.update(actor.user.id, bodyOf<UpdateOwnProfileInput>(req), actor));
  })
);
...
userRouter.get(
  ...
  handler(async (req, res) => ok(res, await userService.getById(paramsOf<{ id: string }>(req).id)))
);
