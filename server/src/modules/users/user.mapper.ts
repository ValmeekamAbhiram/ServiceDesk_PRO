...
import type { CurrentUserDto, UserDto, UserRefDto } from '@shared/types';
import type { Permission } from '@shared/enums';
import { permissionList } from '@/core/authz/permissions';
import type { UserDoc } from '@/models';

/** The least a document needs for the `{ id, name, email, role }` stub. */
export type UserRefSource = Pick<UserDoc, '_id' | 'name' | 'email' | 'role'>;

export type UserSource = UserRefSource &
  Pick<
    UserDoc,
    'status' | 'jobTitle' | 'phone' | 'openTicketCount' | 'assignedTicketCount' | 'createdAt'
  >;

export function toUserRefDto(user: UserRefSource): UserRefDto {
  return {
    id: String(user._id),
    name: user.name,
    email: user.email,
 ...
 * removed still lists.
 */
export function toUserRefDtoOrNull(user: UserRefSource | null | undefined): UserRefDto | null {
  if (!user || typeof user !== 'object' || !('name' in user)) return null;
  return toUserRefDto(user);
}

export function toUserDto(user: UserSource): UserDto {
  return {
    ...toUserRefDto(user),
    status: user.status,
