...
import { isSelf } from '@/core/actor';
import { User, toObjectId } from '@/models';
...
const USER_FIELDS = 'name email role status jobTitle phone openTicketCount assignedTicketCount createdAt version';
const SORT_FIELDS = {
    name: 'name',
    createdAt: 'createdAt',
...
export async function list(query) {
    ...
    const cursor = User.find(filter).select(USER_FIELDS).sort(sort).skip(skip).limit(limit);
 ...
 * everybody out.
 */
function assertNotSelfPrivilegeChange(id, input, actor) {
    if (!isSelf(actor, id))
        return;
 ...
 */
const AUDITED_USER_FIELDS = ['name', 'role', 'status', 'jobTitle', 'phone'];
