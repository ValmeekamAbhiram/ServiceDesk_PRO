...
import { CommentVisibility, Priority, Role, SlaState, TicketStatus, UserStatus, } from '@shared/enums';
import { FixedClock } from '@/core/clock';
import { resolvePermissions } from '@/core/authz/permissions';
import { Asset, Category, User } from '@/models';
import { clearDb, startDb, stopDb } from '@/test/db';
import { listTicketsSchema } from '@/modules/tickets/ticket.schema';
...
async function raise(s, actor, over = {}) {
...
describe('who can see which tickets', () => {
    ...
    it('cannot be widened by asking for somebody else\u2019s tickets', async () => {
        ...
        const page = await tickets.list(listQuery({ requesterId: s.other._id.toString() }), s.asEmployee);
    ...
    it('resolves `scope=mine` per role: assigned for staff, raised for employees', async () => {
        ...
        await tickets.assign(assigned.id, { assigneeId: s.tech._id.toString(), version: assigned.version }, s.asAdmin);
...
describe('the status machine', () => {
    it('walks the table and records every hop on the timeline', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const started = await tickets.changeStatus(ticket.id, { status: TicketStatus.IN_PROGRESS, version: ticket.version }, s.asTech);
        const held = await tickets.changeStatus(started.id, { status: TicketStatus.ON_HOLD, note: 'Waiting on the vendor.', version: started.version }, s.asTech);
    ...
    it('refuses a transition the table does not list', async () => {
        ...
        await expect(tickets.changeStatus(ticket.id, { status: TicketStatus.CLOSED, version: ticket.version }, s.asTech)).rejects.toMatchObject({ statusCode: 409 });
    });
    it('refuses a no-op transition to the status it already has', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await expect(tickets.changeStatus(ticket.id, { status: TicketStatus.OPEN, version: ticket.version }, s.asTech)).rejects.toMatchObject({ statusCode: 409 });
    });
    it('will not resolve a ticket without saying what fixed it', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await expect(tickets.changeStatus(ticket.id, { status: TicketStatus.RESOLVED, version: ticket.version }, s.asTech)).rejects.toMatchObject({ statusCode: 400 });
    });
});
describe('resolving, closing and reopening', () => {
    ...
    async function resolved(s) {
        const ticket = await raise(s, s.asEmployee);
        return tickets.changeStatus(ticket.id, {
    ...
    it('lets the requester reopen, and restarts only the resolution clock', async () => {
        ...
        const after = await tickets.reopen(before.id, { note: 'It powered off again this morning.', version: before.version }, s.asEmployee);
    ...
    it('refuses a reopen from an unrelated employee', async () => {
        ...
        await expect(tickets.reopen(ticket.id, { version: ticket.version }, s.asOther)).rejects.toMatchObject({ statusCode: 404 });
    });
});
describe('concurrent edits', () => {
    it('rejects a write built from a stale version', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await tickets.update(ticket.id, { title: 'Laptop will not boot at all', version: ticket.version }, s.asTech);
        ...
        await expect(tickets.update(ticket.id, { priority: Priority.URGENT, version: ticket.version }, s.asTech)).rejects.toMatchObject({ statusCode: 409 });
    });
    it('does not burn a version on an assignment that changes nothing', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const techId = s.tech._id.toString();
        const assigned = await tickets.assign(ticket.id, { assigneeId: techId, version: ticket.version }, s.asAdmin);
        const again = await tickets.assign(ticket.id, { assigneeId: techId, version: assigned.version }, s.asAdmin);
    ...
    it('clears an assignment when handed an explicit null', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const assigned = await tickets.assign(ticket.id, { assigneeId: s.tech._id.toString(), version: ticket.version }, s.asAdmin);
        const cleared = await tickets.assign(assigned.id, { assigneeId: null, version: assigned.version }, s.asAdmin);
        expect(cleared.assignee).toBeNull();
    });
    it('refuses to assign a ticket to somebody who is not staff', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await expect(tickets.assign(ticket.id, { assigneeId: s.other._id.toString(), version: ticket.version }, s.asAdmin)).rejects.toMatchObject({ statusCode: 400 });
    });
});
describe('changing the priority', () => {
    it('reprices the deadlines from when the ticket was raised, not from now', async () => {
        ...
        const escalated = await tickets.update(ticket.id, { priority: Priority.URGENT, version: ticket.version }, s.asTech);
        ...
        expect(escalated.sla.resolution.budgetMinutes).toBeLessThan(ticket.sla.resolution.budgetMinutes);
    });
...
describe('searching and filtering', () => {
    ...
    it('filters by status and priority', async () => {
        ...
        await tickets.changeStatus(open.id, { status: TicketStatus.IN_PROGRESS, version: open.version }, s.asTech);
...
describe('comments', () => {
    it('records a public reply and marks the first response', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const comment = await tickets.addComment(ticket.id, { body: 'Bringing a spare charger over now.', visibility: CommentVisibility.PUBLIC }, s.asTech);
    ...
    it('does not let an internal note stop the response clock', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const note = await tickets.addComment(ticket.id, { body: 'Vendor RMA is already open under case 4471.', visibility: CommentVisibility.INTERNAL }, s.asTech);
        const after = await tickets.getById(ticket.id, s.asTech);
    ...
    it('does not let the requester answer their own ticket', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        const comment = await tickets.addComment(ticket.id, { body: 'Adding a photograph of the error screen.', visibility: CommentVisibility.PUBLIC }, s.asEmployee);
    ...
    it('hides internal notes from the requester and shows them to staff', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await tickets.addComment(ticket.id, { body: 'Public: engineer assigned.', visibility: CommentVisibility.PUBLIC }, s.asTech);
        await tickets.addComment(ticket.id, { body: 'Internal: this user has broken two chargers already.', visibility: CommentVisibility.INTERNAL }, s.asTech);
    ...
    it('refuses an internal note from somebody without the permission', async () => {
        const s = await seed();
        const ticket = await raise(s, s.asEmployee);
        await expect(tickets.addComment(ticket.id, { body: 'Trying to write an internal note.', visibility: CommentVisibility.INTERNAL }, s.asEmployee)).rejects.toMatchObject({ statusCode: 403 });
    });
});
describe('the denormalised counters', () => {
    it('tracks open tickets per requester, assignee and asset', async () => {
        ...
        await tickets.assign(ticket.id, { assigneeId: s.tech._id.toString(), version: ticket.version }, s.asAdmin);
        ...
        const resolvedTicket = await tickets.changeStatus(ticket.id, {
            status: TicketStatus.RESOLVED,
            resolutionNote: 'Reseated the battery connector.',
            version: current.version,
        }, s.asTech);
        await tickets.changeStatus(resolvedTicket.id, { status: TicketStatus.CLOSED, version: resolvedTicket.version }, s.asTech);
    ...
    it('counts tickets per category, and moves the count when the category changes', async () => {
        ...
        await tickets.update(misfiled.id, { categoryId: software._id.toString(), version: misfiled.version }, s.asTech);
    ...
    it('leaves both category counts alone when the ticket is refiled into a retired category', async () => {
        ...
        const ticket = await raise(s, s.asEmployee);
        await expect(tickets.update(ticket.id, { categoryId: retired._id.toString(), version: ticket.version }, s.asTech)).rejects.toThrow();
        expect((await Category.findById(s.category._id))?.ticketCount).toBe(1);
