...
import { addCommentSchema, assignTicketSchema, changeStatusSchema, createTicketSchema, listCommentsSchema, listTicketsSchema, reopenTicketSchema, ticketIdSchema, updateTicketSchema, } from '@/modules/tickets/ticket.schema';
...
ticketRouter.post('/', requirePermission(Permission.TICKET_CREATE), ...withFiles, validate(createTicketSchema), controller.create);
ticketRouter.get('/:id', canRead, validate(ticketIdSchema), controller.getById);
ticketRouter.patch('/:id', requirePermission(Permission.TICKET_UPDATE), validate(updateTicketSchema), controller.update);
ticketRouter.patch('/:id/status', requirePermission(Permission.TICKET_UPDATE), validate(changeStatusSchema), controller.changeStatus);
ticketRouter.patch('/:id/assign', requirePermission(Permission.TICKET_ASSIGN), validate(assignTicketSchema), controller.assign);
...
ticketRouter.post('/:id/comments', canRead, ...withFiles, validate(addCommentSchema), controller.addComment);
