...
import { ServerEvent, type TicketEventPayload } from '@shared/socket';
import type { Paginated, TicketCommentDto, TicketDto, TicketListItemDto } from '@shared/types';
...
import type { UserRefSource } from '@/modules/users/user.mapper';
import {
  ForbiddenError,
...
/**
 * A loaded, mutable ticket. Spelled out because `TicketDoc` is the plain field shape;
 * the helpers below call `.save()` and rely on Mongoose tracking their assignments.
 */
type TicketRecord = HydratedDocument<TicketDoc>;
...
function auditSnapshot(ticket: TicketRecord): Record<string, unknown> {
  return {
    title: ticket.title,
 ...
 */
export function ticketScopeFilter(actor: ActorContext): FilterQuery<TicketDoc> {
  if (can(actor, Permission.TICKET_READ_ALL)) return {};
 ...
 */
function ticketEvent(ticket: TicketRecord): TicketEventPayload {
  return { ticketId: String(ticket._id), number: ticket.number };
}

export async function ticketMapContext(actor: ActorContext): Promise<TicketMapContext> {
 ...
 */
async function detailById(id: string, actor: ActorContext): Promise<TicketDto> {
  ...
  return toTicketDto(ticket as unknown as TicketSource, ctx, { attachments, actors });
}

/** Every user named in the timeline, fetched once rather than populated per row. */
async function actorsIn(ticket: TicketRecord): Promise<Map<string, UserRefSource>> {
 ...
 * the note itself.
 */
async function attachmentsFor(ticket: TicketRecord, actor: ActorContext) {
  ...
  return rows.map((row) => toAttachmentDto(row as never));
}
...
export async function create(
  ...
  log.info(
    { requestId: actor.requestId, ticketId: String(ticket._id), number, priority },
    'ticket created'
  );
...
const SORT_FIELDS: Record<ListTicketsInput['sortBy'], string> = {
  createdAt: 'createdAt',
  updatedAt: 'updatedAt',
  number: 'number',
 ...
 */
function asTicketNumber(term: string): string | null {
 ...
 * only ever narrow.
 */
function buildListFilter(query: ListTicketsInput, actor: ActorContext): FilterQuery<TicketDoc> {
  const selfId = toObjectId(actor.user.id);
  const staffView = can(actor, Permission.TICKET_READ_ALL);
  const clauses: FilterQuery<TicketDoc>[] = [ticketScopeFilter(actor)];
  ...
  if (query.priority) clauses.push({ priority: { $in: query.priority } });
  if (query.categoryId) clauses.push({ categoryId: toObjectId(query.categoryId) });
  if (query.assigneeId) clauses.push({ assigneeId: toObjectId(query.assigneeId) });
  if (query.requesterId) clauses.push({ requesterId: toObjectId(query.requesterId) });
  if (query.assetId) clauses.push({ assetId: toObjectId(query.assetId) });
  ...
  if (exactNumber) clauses.push({ number: exactNumber });
...
export async function list(
  query: ListTicketsInput,
  actor: ActorContext
): Promise<Paginated<TicketListItemDto>> {
  ...
  if (textTerm) Object.assign(filter, { $text: { $search: textTerm } });
  ...
  const sort: Record<string, SortOrder | { $meta: 'textScore' }> = textTerm
  ...
  const cursor = Ticket.find(filter)
    ...
    .skip(skip)
    .limit(limit);
  if (textTerm) cursor.select({ score: { $meta: 'textScore' } });
  ...
  return toPaginated(
    rows.map((row) => toTicketListItemDto(row as unknown as TicketSource, ctx)),
    page,
    limit,
    total
  );
...
export async function getById(id: string, actor: ActorContext): Promise<TicketDto> {
 ...
 */
async function relinkAsset(ticket: TicketRecord, nextAssetId: string | null): Promise<void> {
  const currentId = ticket.assetId ? String(ticket.assetId) : null;
  if (currentId === nextAssetId) return;
  ...
  if (ticket.assetId) {
    writes.push(
      Asset.updateOne(
        { _id: ticket.assetId },
        { $inc: { ticketCount: -1, openTicketCount: -openDelta } }
      )
    );
  }
  if (next) {
    writes.push(
      Asset.updateOne({ _id: next._id }, { $inc: { ticketCount: 1, openTicketCount: openDelta } })
    );
  }
...
async function shiftOpenCounters(ticket: TicketRecord, delta: number): Promise<void> {
  await Promise.all([
...
export async function update(
  ...
  if (input.assetId !== undefined) await relinkAsset(ticket, input.assetId);
  ...
  if (input.priority !== undefined && input.priority !== ticket.priority) {
    ...
    log.info(
      { requestId: actor.requestId, ticketId: id, from: previous, to: input.priority },
...
export async function assign(
  ...
  if (previousId === input.assigneeId) return detailById(id, actor);
  ...
  log.info(
    { requestId: actor.requestId, ticketId: id, from: previousId, to: input.assigneeId },
    input.assigneeId ? 'ticket assigned' : 'ticket unassigned'
  );
...
/* ───────────────────────────── the status machine ───────────────────────────
 * One implementation, two entry points. `changeStatus` is the staff workflow and its
 * route demands `ticket:update`; `reopen` is the requester saying "this is not fixed"
 * and its route cannot demand that permission, because an employee does not have it.
 * Both funnel through `applyStatusChange`, so the transition table, the SLA effects
 * and the timeline entry are written once.
 * ------------------------------------------------------------------------- */
...
async function applyStatusChange(
  ...
  if (wasOpen !== nowOpen) await shiftOpenCounters(ticket, nowOpen ? 1 : -1);

  log.info(
    { requestId: actor.requestId, ticketId: String(ticket._id), from, to },
...
export async function changeStatus(
  ...
  await applyStatusChange(
    ticket,
    input.status,
    { note: input.note, resolutionNote: input.resolutionNote },
    actor
  );
  return detailById(id, actor);
}
...
export async function addComment(
  ...
  if (
    input.visibility === CommentVisibility.INTERNAL &&
  ...
  const attachments =
    files.length > 0
      ? await recordAll(
          files,
          { ticketId: ticket._id, commentId: comment._id, visibility: input.visibility },
          actor
        )
      : [];
...
export async function listComments(
  ...
  const byComment = new Map<string, ReturnType<typeof toAttachmentDto>[]>();
  for (const file of files) {
    const key = String(file.commentId);
    const bucket = byComment.get(key);
    if (bucket) bucket.push(toAttachmentDto(file as never));
    else byComment.set(key, [toAttachmentDto(file as never)]);
  }

  return toPaginated(
    rows.map((row) =>
      toTicketCommentDto(row as never, byComment.get(String(row._id)) ?? [])
    ),
    page,
    limit,
    total
