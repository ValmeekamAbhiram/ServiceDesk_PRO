 ...
 */
export function applyMongooseDefaults() {
 ...
 */
function probeTcp(uri, timeoutMs = 800) {
    let host = '127.0.0.1';
    let port = 27017;
    try {
        const parsed = new URL(uri);
...
export async function connectDb() {
    if (connectionInfo && mongoose.connection.readyState === 1)
    ...
    if (env.inMemoryDb === 'always') {
        ...
        log.warn('Using an in-process MongoDB (USE_IN_MEMORY_DB=true). All data is discarded when this process exits.');
    ...
    if (reachable) {
        try {
            ...
            if (env.inMemoryDb === 'never')
                throw error;
            log.warn({ target: describeUri(env.MONGO_URI), err: error }, 'Could not connect to MongoDB; falling back to an in-process instance.');
        }
    }
    else {
        log.warn({ target: describeUri(env.MONGO_URI) }, 'Nothing is listening on MONGO_URI; falling back to an in-process MongoDB.');
    }
    const uri = await startMemoryServer();
    await mongoose.connect(uri, { ...options, autoIndex: true });
    connectionInfo = { kind: 'in-memory', target: describeUri(uri), ephemeral: true };
    log.warn('Running on an ephemeral in-process MongoDB. Data will NOT survive a restart; ' +
 ...
 */
export function registerTestConnection(target) {
