...
import { DemoClock, RealClock } from '@/core/clock';
 ...
 */
let current = env.demoMode ? new DemoClock(0) : new RealClock();
...
export function setAbsolute(instant) {
    const clock = requireDemoClock();
...
export function formatBusinessLocal(instant) {
 ...
 */
export function offsetLabel(offsetMs) {
    if (offsetMs === 0)
        return 'real time';
