...
const baseOptions: LoggerOptions = {
  ...
  hooks: {
    ...
    logMethod(args, method, level) {
      if (quietBelow !== null && level < quietBelow) return;
...
let quietBelow: number | null = null;
 ...
 */
export async function withQuietLogs<T>(run: () => Promise<T>): Promise<T> {
...
function createLogger(): Logger {
  ...
  if (!wantsPretty) return pino(baseOptions);
