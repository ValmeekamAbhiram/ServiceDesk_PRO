...
import { z } from 'zod';
import type { PublicConfigDto } from '@shared/types';
...
function normaliseRawEnv(source: NodeJS.ProcessEnv): Record<string, string> {
 ...
 */
function zLowerEnum<const T extends readonly [string, ...string[]]>(
  values: T,
  defaultValue: T[number]
) {
  return z.preprocess(
    (v) => (v === undefined ? defaultValue : String(v).trim().toLowerCase()),
    z.enum(values)
  );
}
...
const raw: ParsedEnv = parsed.data;
...
function looksLikeDevSecret(secret: string): boolean {
...
if (isProduction) {
  ...
  if (looksLikeDevSecret(raw.JWT_SECRET)) offenders.push('JWT_SECRET');
  if (looksLikeDevSecret(raw.JWT_REFRESH_SECRET)) offenders.push('JWT_REFRESH_SECRET');
  if (offenders.length) {
    throw new Error(
      `Refusing to start in production with the development ${offenders.join(' and ')}. ` +
  ...
  if (raw.JWT_SECRET === raw.JWT_REFRESH_SECRET) {
    throw new Error(
      'JWT_SECRET and JWT_REFRESH_SECRET must differ — a leaked access token must not be usable as a refresh token.'
    );
  }
  if (raw.USE_IN_MEMORY_DB === 'true') {
    throw new Error(
      'USE_IN_MEMORY_DB=true cannot be used in production: all data is lost when the process exits. Set MONGO_URI instead.'
    );
  }
...
const inMemoryDb: 'auto' | 'always' | 'never' = isProduction
  ? 'never'
...
const aiProvider: 'anthropic' | 'heuristic' =
  raw.AI_ENABLED && raw.AI_PROVIDER !== 'heuristic' && aiHasKey ? 'anthropic' : 'heuristic';

if (raw.AI_ENABLED && raw.AI_PROVIDER !== 'heuristic' && !aiHasKey) {
  warnings.push(
    'AI_API_KEY is empty — ticket suggestions will use the offline keyword classifier.'
  );
}
