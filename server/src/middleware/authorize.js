 ...
 * directory.
 */
export function requireResourceOwnership(options = {}) {
    const param = options.param ?? 'id';
    const overrides = options.overridePermissions ?? [];
    return function requireOwnershipMiddleware(req, _res, next) {
        try {
            const actor = actorOf(req);
 ...
 */
export function requireDemoMode(...permissions) {
    return function requireDemoModeMiddleware(req, _res, next) {
        try {
            if (!env.demoMode) {
