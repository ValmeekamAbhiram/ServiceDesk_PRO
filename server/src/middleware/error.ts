 ...
 */
function isClientAbort(error: unknown, res: Response): boolean {
  const code = (error as { code?: string } | null)?.code;
  if (code !== undefined && CLIENT_ABORT_CODES.has(code)) return true;
  return res.socket?.destroyed === true || !res.writable;
}

/** Assemble the wire body, applying the `expose` rule. */
function buildBody(error: AppError, requestId: string): ApiError {
  ...
  if (error.fields?.length) body.error.fields = error.fields;
   ...
   */
  if (error.expose && error.meta) Object.assign(body.error, error.meta);
  ...
  return function errorMiddleware(
    ...
    if (res.headersSent) {
      logger.error(
        ...
        'Error after response started; connection destroyed.'
      );
      res.destroy();
      return;
    }
