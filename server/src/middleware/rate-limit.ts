...
interface Bucket {
  /** Request timestamps inside the window, ascending. */
  hits: number[];
  /** Latest hit, used by the sweeper to drop idle buckets. */
  lastSeen: number;
}
...
export interface RateLimitOptions {
  ...
  max: number;
  /** Label used in logs and to namespace keys between limiters. */
  name: string;
  /** Do not count 2xx/3xx responses. See the note on the auth limiter. */
  skipSuccessful?: boolean;
  /** Extra key material, e.g. the submitted email on the login route. */
...
function identityKey(req: Request, options: RateLimitOptions): string {
    ...
    if (options.skipSuccessful) {
      res.on('finish', () => {
        if (res.statusCode < 400) {
          ...
          if (index >= 0) current.hits.splice(index, 1);
        }
      });
    }
...
export const globalRateLimit = (): RequestHandler =>
  rateLimit({ name: 'global', max: env.RATE_LIMIT_MAX });
...
export const uploadRateLimit = (): RequestHandler =>
  rateLimit({ name: 'upload', max: 60, message: 'Too many uploads. Please wait a moment.' });
