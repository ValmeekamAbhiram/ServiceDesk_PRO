...
function toAuthenticatedUser(user, sessionId) {
 ...
 */
export async function authenticateToken(token, nowMs) {
    const claims = verifyAccessToken(token, nowMs);
    const [user, session] = await Promise.all([
        User.findById(claims.sub).select(USER_PROJECTION).lean().exec(),
    ...
    if (user.passwordChangedAt && typeof claims.iat === 'number') {
        ...
        if (claims.iat < changedAtSeconds)
            return fail('token predates password change');
    }
