...
async function seedEphemeralDatabase(): Promise<void> {
  ...
  log.info(
    { ...result.counts, durationMs: result.durationMs },
    'Seeded the in-memory database with demo data'
  );
...
async function main(): Promise<void> {
  ...
  if (db.ephemeral && !env.isProduction) await seedEphemeralDatabase();
  ...
  log.info(
    { port: env.PORT, env: env.NODE_ENV, demoMode: env.demoMode, version: env.appVersion },
    `ServiceDesk Pro API listening on ${env.SERVER_URL}`
  );
  ...
  const shutdown = (signal: string): void => {
    ...
    void closeSocketServer(io)
      .catch((error: unknown) => log.error({ err: error }, 'Error closing the real-time channel'))
      .then(() => disconnectDb())
      .catch((error: unknown) => log.error({ err: error }, 'Error closing the database'))
