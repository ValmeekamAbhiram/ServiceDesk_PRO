...
import { optionsFromEnv, runSeed, type SeedOptions } from '@/seed/seed';
...
interface ParsedArgs {
  ...
  help: boolean;
  /** Human-readable complaints. Any at all and the run is refused before connecting. */
  problems: string[];
}

function parseArgs(argv: readonly string[]): ParsedArgs {
  const overrides: Partial<SeedOptions> = {};
...
function summarise(label: string, value: number): string {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (options.minimal) out('Minimal run — no assets, articles or tickets.');
    out();
