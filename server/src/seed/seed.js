...
import { AssetStatus, AssetType, CommentVisibility, Role, TicketStatus, UserStatus, } from '@shared/enums';
import { DAY_MS } from '@shared/utils';
import { env } from '@/config/env';
import { moduleLogger } from '@/config/logger';
import { clockAt, FixedClock } from '@/core/clock';
import { resolvePermissions } from '@/core/authz/permissions';
import { ASSET_SEQUENCE, Asset, Attachment, AuditLog, Category, Counter, KnowledgeArticle, Notification, Session, SlaPolicy, SystemSettings, TICKET_SEQUENCE, Ticket, TicketComment, User, resetSequence, toObjectId, } from '@/models';
import { getSlaPolicy, invalidateSlaPolicyCache } from '@/modules/sla/sla-policy.service';
import { getSystemSettings, invalidateSystemSettingsCache, } from '@/modules/settings/settings.service';
...
export function optionsFromEnv(overrides = {}) {
    return {
        reset: false,
        minimal: false,
...
async function backdate(model, id, createdAt, updatedAt = createdAt) {
...
async function seedUsers(options) {
    ...
    const docs = await User.insertMany(wanted.map((person) => ({
        name: person.name,
        email: person.email,
        passwordHash,
        role: person.role,
    ...
    return docs.map((doc) => toSeedUser(doc));
}
...
async function seedCategories() {
    const docs = await Category.insertMany(CATEGORIES.map((category) => ({
        name: category.name,
        slug: category.slug,
 ...
 */
async function seedSingletons(now) {
    await getSlaPolicy();
...
async function seedAssets(admin, people, options, rng, now) {
    const ids = [];
    ...
    for (const spec of ASSETS) {
        ...
        ids.push(asset.id);
        if (holder)
            byHolder.set(holder.id, asset.id);
 ...
 */
async function seedArticles(admin, author, categories, now) {
    let created = 0;
 ...
 */
function outcomeFor(ageDays, rng) {
    if (ageDays < 2) {
        return rng.weighted([
            ['OPEN_UNTOUCHED', 4],
            ['OPEN_ANSWERED', 3],
 ...
 */
function responseDelayMinutes(targetMinutes, late, rng) {
    if (late)
        return targetMinutes + rng.int(targetMinutes, targetMinutes * 5);
 ...
 */
async function currentVersion(ticketId) {
    const doc = await Ticket.findById(toObjectId(ticketId)).select('version').lean();
    if (!doc)
        throw new Error(`Seeded ticket ${ticketId} disappeared mid-build.`);
    return doc.version;
}
/**
 ...
 */
async function buildTicket(template, requester, technician, categoryId, assetId, createdAt, outcome, 
...
assigner, policy, rng) {
    ...
    const responseMinutes = responseDelayMinutes(policy.targets[created.priority].responseMinutes, late, rng);
    ...
    const assigned = await ticketService.assign(ticketId, { assigneeId: technician.id, version }, assignerActor);
    version = assigned.version;
    let lastEventAt = clock.now();
    clock.advanceMinutes(Math.max(1, responseMinutes - pickupMinutes));
    const firstReply = await ticketService.addComment(ticketId, { body: template.replies[0] ?? 'Looking into this now.', visibility: CommentVisibility.PUBLIC }, techActor);
    comments += 1;
    ...
    if (template.notes?.[0]) {
        ...
        const note = await ticketService.addComment(ticketId, { body: template.notes[0], visibility: CommentVisibility.INTERNAL }, techActor);
        comments += 1;
        version = await currentVersion(ticketId);
        lastEventAt = clock.now();
    ...
    const working = await ticketService.changeStatus(ticketId, { status: TicketStatus.IN_PROGRESS, version }, techActor);
    ...
    if (outcome === 'ON_HOLD') {
        ...
        const held = await ticketService.changeStatus(ticketId, {
    ...
    if (template.replies[1]) {
        ...
        const followUp = await ticketService.addComment(ticketId, { body: template.replies[1], visibility: CommentVisibility.PUBLIC }, techActor);
        comments += 1;
        version = await currentVersion(ticketId);
        lastEventAt = clock.now();
    ...
    const resolved = await ticketService.changeStatus(ticketId, { status: TicketStatus.RESOLVED, resolutionNote: template.resolution, version }, techActor);
    ...
    if (outcome === 'REOPENED') {
        ...
        const reopened = await ticketService.reopen(ticketId, { note: 'This has come back today, exactly as before.', version }, { ...requesterActor, clock });
 ...
 */
async function settleNotifications(watermark, eventAt, now) {
    ...
    await raw(Notification).updateMany({ createdAt: { $gte: watermark } }, {
        $set: {
            createdAt: eventAt,
            updatedAt: eventAt,
            read,
 ...
 */
async function seedTickets(people, categories, assets, options, rng, now) {
    ...
    const requesterWeights = [
        ...employees.map((person, index) => [person, index === 0 ? employees.length : 1]),
        [admin, 2],
    ];
    ...
    const technicianWeights = technicians.map((person, index) => [person, Math.max(1, technicians.length - index)]);
    ...
    const instants = Array.from({ length: options.ticketCount }, () => rng.businessInstant(windowStart, now, 1.6)).sort((a, b) => a.getTime() - b.getTime());
    for (const [index, createdAt] of instants.entries()) {
        ...
        if (!category)
            throw new Error(`Template references unknown category "${template.category}".`);
        ...
        const assetId = template.linkAsset === 'requester'
            ? assets.byHolder.get(requester.id) ?? null
            : template.linkAsset === 'shared' && assets.shared.length > 0
                ? rng.pick(assets.shared)
                : null;
        const watermark = new Date();
        const result = await buildTicket(template, requester, technician, String(category._id), assetId, createdAt, outcome, triagedByAdmin ? admin : technician, policy, rng);
        await settleNotifications(watermark, result.lastEventAt, now);
        comments += result.comments;
        built += 1;
        if (built % 25 === 0)
            log.info({ built, of: options.ticketCount }, 'Tickets building');
    }
...
export async function runSeed(options) {
    ...
    if (options.reset) {
        ...
        if (existing > 0) {
            throw new Error(`Database already holds ${existing} user${existing === 1 ? '' : 's'}. ` +
    ...
    let assets = { count: 0, byHolder: new Map(), shared: [] };
    let articles = 0;
    let totals = { tickets: 0, comments: 0 };
    if (!options.minimal) {
