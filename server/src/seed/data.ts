...
export interface SeedCategory {
  ...
  sortOrder: number;
  keywords: string[];
  /** Retired, to prove the admin screen deactivates rather than deletes. */
  inactive?: boolean;
}
...
export interface SeedAsset {
  ...
  model: string;
  location: string;
  /** Whole months from the seed run: negative is the past. Drives warranty radar. */
  warrantyMonths: number | null;
  purchaseMonthsAgo: number | null;
  purchaseCost: number | null;
  /** Index into `PEOPLE`, or null for stock and infrastructure. */
  holder: number | null;
}
...
export interface SeedArticle {
  ...
  summary: string;
  body: string;
  /** Category slug from `CATEGORIES`, or null for a general article. */
  category: string | null;
  tags: string[];
  /** Left unpublished, so the review boundary is visible in the seeded data. */
  draft?: boolean;
  /** Roughly how many people have opened it; sorts the "most read" list. */
  views: number;
}
...
export interface SeedTicketTemplate {
  /** Category slug. Resolved to an id at seed time — never an index into the list. */
  category: string;
  title: string;
  description: string;
  /** Overrides the category default. Left off, the category decides. */
  priority?: Priority;
  /**
   * Which asset, if any, the ticket is about. `'requester'` links the reporter's own
   * machine; `'shared'` links a printer, display or switch that belongs to nobody.
   * A boolean would have been enough right up until the warehouse printer ticket linked
   * somebody's laptop.
   */
  linkAsset?: 'requester' | 'shared';
  /** Public replies, in order. The first one stops the SLA response clock. */
  replies: readonly string[];
  /** Staff-only notes, interleaved after the replies. */
  notes?: readonly string[];
  /** Required to move a ticket to RESOLVED, so every template carries one. */
  resolution: string;
}
...
export const TICKET_TEMPLATES: readonly SeedTicketTemplate[] = [
  {
    ...
    description:
      'Since Monday the VPN drops roughly every five minutes. It reconnects on its own but anything I have open on the file server fails in the meantime. Home broadband is otherwise fine — video calls on the same connection are stable.',
    replies: [
  ...
  },
  {
    ...
    description:
      'Nobody in Ellora or Hampi can get online — laptops show connected to the wifi but no internet. Wired ports in the same rooms work. We have a client call in an hour.',
  ...
  },
  {
    ...
    description:
      'Downloads that take seconds upstairs take minutes here. It has been getting worse since the new racking went in.',
    replies: [
  ...
  },
  {
    ...
    description:
      'Powers on, shows the logo, then a blue screen with INACCESSIBLE_BOOT_DEVICE and restarts in a loop. It was working when I shut it down last night. I have a report due today.',
  ...
  },
  {
    ...
    description:
      'The external monitor flickers about once a minute, but only through the dock. Straight into the laptop it is perfectly steady.',
    linkAsset: 'requester',
    replies: [
  ...
  },
  {
    ...
    description:
      'Down from most of a day when it was new. It also runs hot enough that I cannot keep it on my lap.',
    linkAsset: 'requester',
    replies: [
  ...
  },
  {
    ...
    description:
      'Excel closes without a message roughly ten seconds after opening it. Other spreadsheets are fine. The file opens on my colleague\u2019s machine.',
    priority: Priority.HIGH,
    replies: [
  ...
  },
  {
    ...
    description:
      'Sharing shows a black rectangle to everyone else. Camera and microphone work. Started after last week\u2019s update.',
    replies: [
  ...
  },
  {
    ...
    description:
      'Finance is moving the monthly pack to Power BI and I need the desktop application to edit the reports.',
  ...
  },
  {
    ...
    description:
      'I mistyped my password a few times and now it says my account is locked. I have a payroll deadline this afternoon.',
    priority: Priority.HIGH,
    replies: [
  ...
  },
  {
    ...
    description:
      'The F: drive shows a red cross and says access denied. It worked last week. Colleagues on the same team can still open it.',
    replies: [
  ...
  },
  {
    ...
    description:
      'Sana Qureshi joins the marketing team on Monday. She needs email, the shared drive and access to the design tools.',
    priority: Priority.LOW,
    replies: [
  ...
  },
  {
    ...
    description:
      'The printer shows offline on my machine although its own screen says ready. Nobody on the floor can print.',
    replies: [
  ...
  },
  {
    ...
    description:
      'Scan to email works but every page comes through completely white. Photocopying on the same machine is fine.',
    priority: Priority.LOW,
    replies: [
  ...
  },
  {
    ...
    description:
      'Jams every third or fourth label. We are hand-writing labels to keep dispatch moving.',
    priority: Priority.HIGH,
    linkAsset: 'shared',
    replies: [
  ...
  },
  {
    ...
    description:
      'Outlook says my mailbox is full. I can receive but not send. Most of it is old attachments I do not need.',
    replies: [
  ...
  },
  {
    ...
    description:
      'An email claiming to be from a supplier asks us to update their bank account before the next payment run. It looks close to their real address but not quite right. I have not replied.',
    priority: Priority.URGENT,
    replies: [
  ...
  },
  {
    ...
    description:
      'External attendees say they never receive my invitations. Internal colleagues get them fine.',
    replies: [
  ...
  },
  {
    ...
    description:
      'The screen in Ellora says no signal whatever laptop we plug in. It was working yesterday.',
    priority: Priority.HIGH,
    linkAsset: 'shared',
    replies: [
