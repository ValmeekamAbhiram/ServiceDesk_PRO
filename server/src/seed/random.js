...
export class Rng {
    ...
    pick(items) {
        if (items.length === 0)
            throw new Error('Rng.pick was given an empty list.');
     ...
     */
    businessInstant(from, to, recencyBias = 1) {
        ...
        if (this.chance(0.1))
            return at; // out of hours on purpose
        const day = at.getDay();
        if (day === 0)
            at.setDate(at.getDate() + 1); // Sunday -> Monday
        if (day === 6)
            at.setDate(at.getDate() + 2); // Saturday -> Monday
