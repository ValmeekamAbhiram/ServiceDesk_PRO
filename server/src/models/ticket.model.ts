...
export interface TicketSlaTarget {
  /**
   * When this clock started. An input, per target — a reopen restarts the resolution
   * clock and leaves the response clock alone, so after that the two differ.
   */
  startedAt: Date | null;
  /** Business minutes allowed, snapshotted from the policy. An input. */
  budgetMinutes: number;
  /** Absolute deadline in business time. An input — never recomputed. */
  dueAt: Date | null;
  /** When the target was actually hit. */
  metAt: Date | null;
  /** Cached projection, refreshed by the monitor. See the header. */
  state: SlaState;
}

export interface TicketDoc {
  _id: Types.ObjectId;
  /** `TKT-000001`. Unique, human-quotable. */
  number: string;
  title: string;
  description: string;
  status: TicketStatus;
  priority: Priority;
  /**
   * `PRIORITY_RANK[priority]`, denormalised so "most urgent first" is an index scan.
   * Mongo would otherwise sort the priority *strings*, which puts HIGH before LOW
   * before MEDIUM — alphabetical, and silently wrong. Kept in step by the
   * `pre('validate')` hook below, so no call site can forget it.
   */
  priorityRank: number;
  categoryId: Types.ObjectId;
  requesterId: Types.ObjectId;
  assigneeId: Types.ObjectId | null;
  /** The asset this is about, if any. Drives "tickets for this laptop". */
  assetId: Types.ObjectId | null;

  statusHistory: TicketStatusChange[];

  sla: {
    /** Policy name at the time the deadlines were set. */
    policyName: string;
    response: TicketSlaTarget;
    resolution: TicketSlaTarget;
  };

  /** Set by the first staff comment; stops the response clock. */
  ...
  /** Denormalised so a list row needs no per-ticket count query. */
  commentCount: number;
...
const statusChangeSchema = new Schema<TicketStatusChange>(
  {
...
const slaTargetSchema = new Schema<TicketSlaTarget>(
  {
 ...
 */
ticketSchema.index(
  { 'sla.resolution.state': 1, 'sla.resolution.dueAt': 1 },
  {
    name: 'sla_sweep',
 ...
 */
ticketSchema.index(
  { number: 'text', title: 'text', description: 'text' },
  { name: 'ticket_search', weights: { number: 20, title: 10, description: 2 } }
);

export const Ticket = defineModel<TicketDoc>('Ticket', ticketSchema);
