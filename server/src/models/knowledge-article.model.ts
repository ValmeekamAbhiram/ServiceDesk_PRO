...
export interface KnowledgeArticleDoc {
  _id: Types.ObjectId;
  title: string;
  /** URL-safe, unique. Generated from the title, kept stable after publishing. */
  slug: string;
  /** One or two sentences shown in search results and on cards. */
  summary: string;
  /** Markdown. Sanitised at render time, never stored as HTML. */
  body: string;
...
knowledgeArticleSchema.index(
  { title: 'text', summary: 'text', tags: 'text', body: 'text' },
  { name: 'article_search', weights: { title: 20, tags: 12, summary: 8, body: 2 } }
);

export const KnowledgeArticle = defineModel<KnowledgeArticleDoc>(
  'KnowledgeArticle',
