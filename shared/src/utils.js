...
export function parseMinuteOfDay(hhmm) {
    const m = /^(\d{1,2}):(\d{2})$/.exec(String(hhmm).trim());
    if (!m)
        throw new Error(`Invalid time-of-day "${hhmm}" (expected HH:MM)`);
    const h = Number(m[1]);
...
export function relativeTime(iso, nowMs) {
    ...
    if (s < 3600)
        return pick(s / 60, 'minute');
    if (s < 86400)
        return pick(s / 3600, 'hour');
    if (s < 86400 * 30)
        return pick(s / 86400, 'day');
    if (s < 86400 * 365)
        return pick(s / (86400 * 30), 'month');
...
/* ─────────────────────────────── Text ──────────────────────────────────── */
export function truncate(input, max = 140) {
...
export function ageInYears(from, to) {
